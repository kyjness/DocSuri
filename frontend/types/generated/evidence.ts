/* GENERATED from shared/dtos/evidence.schema.json by `pnpm gen:types` — do not edit by hand.
 ...
 */
export interface EvidenceResult1 {
  /**
   * ok 고정(성공). Trace: FR-5.
   */
  state: "ok";
  ...
  /**
   * 판단 산문(문장 단위) + 기계 검사 결과. 근거 번호는 claims의 1-기반 순서를 가리키며 근거표 행 번호와 같은 출처다. 근거가 0건이면 null. 하위호환을 위해 선택 필드.
   */
...
export interface SourceRef {
   ...
   */
  anchorType?: "paragraph" | "list" | "code" | "table" | "figure" | "formula";
  /**
   ...
   */
  sourceScope?: "fulltext" | "abstract" | "figure";
}
/**
...
export interface EvidenceCoverage {
  ...
  paperCount: number;
  /**
   * 자동 검색 시 사용된 쿼리(auto·mixed scope). explicit scope이면 생략. v2 자율 루프는 질의를 여러 번 설계하므로 대표 질의(또는 요약)를 싣는다.
   */
  queryUsed?: string;
  /**
   * 실제로 확인(본문·초록 열람)한 논문 수(선택, v2). candidates와 함께 '탐색이 어디까지 갔는지'를 사용자에게 알린다. Trace: FR-37 v2.
   */
  examined?: number;
  /**
   * 탐색 중 발견한 후보 논문 수(선택, v2). examined < candidates이면 탐색이 완결되지 않은 것이다. Trace: FR-37 v2.
   */
  candidates?: number;
  /**
...
export interface AnswerSegment {
  /**
   * 문장 본문. 인용 번호는 refs가 권위이므로 text에 중복해 넣지 않는다.
   */
  text: string;
  /**
   * 이 문장이 근거로 삼은 claims의 1-기반 번호. kind=synthesis면 반드시 빈 배열이다(§4.3 A1·A2 강등이 refs를 비우고 kind를 synthesis로 바꾼다). 번호가 실재하는지는 §4.3 A1이 판정한다 — 스키마는 범위를 이중으로 강제하지 않는다(판정 지점이 둘이 되면 어긋난다).
   */
  refs: number[];
...
export interface AnswerChecks {
  /**
   * A1·A2로 종합 강등된 문장 수. 강등은 거부가 아니다 — 문장은 남되 '기계가 확인함' 표시를 잃는다.
   */
  demoted: number;
  /**
   * 거부되어 재생성을 거쳤는가(시작값 1회).
   */
  regenerated: boolean;
  /**
   * 재생성도 거부되어 결정론 이어붙이기로 떨어졌는가. true면 판단 없이 근거만 있는 답이다 — 검사를 못 통과한 판단은 화면에 가지 않는다(C-2 fail-closed).
   */
  fallback: boolean;
}
/**
 ...
 */
export interface EvidenceAbstainResult {
  /**
