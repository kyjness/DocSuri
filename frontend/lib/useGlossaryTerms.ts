...
// termFrom→termTo map, to pre-fill the badge editor. Pre-fill is optional, so any failure
// (network, 401 when anonymous, 503) degrades silently to an empty map. `setTerm` keeps the
// map in sync after a save so reopening a badge shows the just-saved value without a refetch.
