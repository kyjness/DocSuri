...
// The macro fallback set, TeX package list, author-macro normalizer, and LaTeX preprocessor live in
// `./mathEngine` — a pure, React-free module — so the headless corpus render sweep
// (`scripts/audit-math-render.mjs`) shares the EXACT same rules and a fix reaches both paths. This
// file keeps only the browser engine (lazy chunk load) and the React render spans. Re-export the
// type so existing consumers (DocModelViewer, …) keep importing it from here.
export type { MathMacros };
