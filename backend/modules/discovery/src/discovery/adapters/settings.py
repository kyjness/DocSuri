...
class DiscoverySettings:
    ...
    # The query-embedding model. Bedrock/Cohere is the only embedding path (U1 writes with the
    # same one), so reader and index share a space by construction — what still has to match is
    # the MODEL, which is why the index carries an embedding manifest (vector-spec §4).
    ...
    def search_enabled(self) -> bool:
        """True when the real read path can be wired: cluster + an embedding model."""
