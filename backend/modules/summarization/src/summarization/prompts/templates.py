...
def build_summary_prompt(
    ...
    system = (
        ...
        # 재현성은 **없다는 것도 정보**다. 그런데 "근거가 없으면 비워라"(위 첫 규칙)만 있으면
        # 모델이 빈 문자열을 내고, 화면에서 "논문에 공개 언급이 없다"와 "모델이 답을 안 했다"가
        # 같은 모양이 된다. 실제로 그 구분이 안 되는 것이 결함으로 잡혔다(2026-08-25).
        # 그래서 이 두 칸만 예외를 둔다 — 확인했는데 없으면 그렇게 **적으라고** 시킨다.
        "- reproducibility.code·data는 비우지 마라. 원문에 공개 저장소·데이터셋 언급이 있으면"
        " 그것을 쓰고, 찾아봤는데 **없으면 '논문에 언급 없음'이라고 정확히 그렇게 써라**."
        " 이 두 칸은 '없다'는 사실 자체가 답이다.\n"
