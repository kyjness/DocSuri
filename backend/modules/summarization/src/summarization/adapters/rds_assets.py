...
class RdsS3AssetReader:
    ...
    def list_assets(self, paper_id: str, version: int) -> Sequence[StoredAsset]:
        # Every kind U1 attaches a doc-model assetRef to, because this manifest is the only place
        # the viewer can turn an assetId into a signed url. Excluding "formula" here left the
        # FormulaBlock image fallback (docmodel.schema DocAssetRef, PDF path) unable to resolve
        # its own crop, so an equation GROBID gave no LaTeX for rendered as a bare placeholder
        # while its webp sat in S3 unreachable. Consumers key by assetId, so a kind they don't
        # render is inert — the narrower list bought nothing and cost every PDF-path equation.
        sql = (
            "SELECT asset_id, type, ordinal, caption, source_mode, object_ref, page_ref, bbox "
            "FROM paper_asset WHERE paper_id = %s AND version = %s "
            "AND type IN ('figure', 'table', 'formula') ORDER BY type, ordinal"
        )
