...
class _Sqs:
    ...
    def send_message(self, *, QueueUrl: str, MessageBody: str) -> None:  # noqa: N803 — boto3 계약
        if self.fails:
...
def test_a_non_arxiv_paper_is_never_queued():
    ...
    _queue(sqs).enqueue_index("doi:10.1145/abc")
    _queue(sqs).enqueue_index("userdoc:abc")
...
def test_different_versions_are_different_papers():
    ...
    queue.enqueue_index("arxiv:2304.10557v1")
    queue.enqueue_index("arxiv:2304.10557v3")
...
def _run(index_queue: Any, state_setup) -> Any:
    ...
    try:
        return runner.run(
            AgentRunContext(owner_id="o", session_id="s", turn_id="t"),
            EvidenceRequest(topic="q"),
        )
    finally:
        EvidenceTurnRunner._build_registry = original  # type: ignore[method-assign]


def _seed(state: LoopState) -> None:
    state.examine(
