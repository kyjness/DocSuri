...
| **FR-31** | **novelty 검색·외부 탐색 [U12]** *(2026-07-18 v2 개정 — 메커니즘 중립화)*: U2 `full` 검색을 재사용해 내부 Corpus 유사 연구를 확장 검색하고, 외부 탐색은 특정 메커니즘에 고정하지 않는 **외부 탐색 어댑터**(일반 API 또는 MCP 서버)를 통해 수행한다. v1 범위는 GitHub 검색 + 관련 데이터셋 검색이며, 외부 arXiv 검색은 MCP 연동 단계에서 추가한다. | EvidenceFormationPort 결과를 1차 근거로 삼고 U2 `full` 검색은 누락 보강/유사 논문 확장에 사용한다. novelty 전용 인덱스나 별도 랭킹 로직을 만들지 않는다. GitHub는 구현체·baseline·benchmark·license 단서만 추출하고 품질 점수/재현 가능 판정은 하지 않는다. 데이터셋은 이름·URL·라이선스·접근성·태스크·metric 후보를 산출한다. 외부로 나가는 도구는 도구별 payload allowlist를 어댑터가 기계식으로 강제한다(BR-NV6 승계). 뉴스 검색은 제외 유지. |
...
- **NFR-R3 [U12]** novelty Agent 외부 의존성(U2 `full`, EvidenceFormationPort, 외부 탐색 어댑터[GitHub·데이터셋·향후 arXiv MCP], Notion, LLM)은 source별 실패/저하를 분리한다 *(2026-07-18 v2 개정: Agent-Browser → 외부 탐색 어댑터)*. 한 source 실패가 전체 job 실패로 전파되지 않으며, 성공한 source만으로 부분 산출물을 제공한다. 루프에서는 도구 실패가 오류 결과로 에이전트에 반환되어 대체 경로 판단 기회를 가진 뒤, 지속 실패 시 해당 source를 저하로 기록한다.
