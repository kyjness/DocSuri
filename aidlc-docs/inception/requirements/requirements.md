...
> **개정 (2026-07-18) — U12 novelty v2 재설계(자율 에이전트) 편입**: 아키텍처 질문지 `requirement-verification-questions-agent-rearchitecture.md`(Q1~Q8), 기능 정의 질문지 `requirement-verification-questions-novelty-v2-function.md`(Q1~Q7), 유닛 질문 게이트 `construction/plans/novelty-agent-v2-functional-design-plan.md`(Q1~Q14) 확정을 반영한다. **기능 재정의**: U12의 기본 임무를 **조사 + 여백 분석**(무엇이 이미 연구됐고 어디가 비어 있는지, 근거 링크 필수)으로 재정의(FR-32 개정)하고, 방향 제안·실험 계획은 대화 요청 시 **온디맨드** 생성(FR-33 개정), 원고 위험 신호는 v2 범위에서 폐기(FR-34 폐기), 상호작용은 채팅 모드 + 잡 + 대화 스티어링. **오케스트레이션 교체**: 고정 상태머신 → **완전 자율 도구 호출 루프**(FR-30 개정), 진행 표시는 거시 상태 소수 + 결정 트레이스 파생 활동 피드(FR-35 개정), 외부 탐색 메커니즘 중립화(FR-31 개정 — Agent-Browser 고정 해제, 외부 탐색 어댑터·MCP 포함). **신규 등재**: FR-44(세션 메모리) · FR-45(루프 예산·반복 상한) · FR-46(결정 추적성) · FR-47(근거 대상 확장 — DocModel 표·그림·수식 객체 앵커 [U11/U12], 구체 설계는 문헌탐색 에이전트화 게이트). **불변**: C-2 날조 금지·결정론 검증, U6 비용 단일 권위, Notion 승인 게이트, `EvidenceFormationPort`/`SourceRef` 소비·재구현 금지. 최종 목표 구조는 supervisor–서브 에이전트(유지보수 로드맵 ⑤~⑦), 화면·API 계약은 새 산출물 기준 재설계 허용. 설계 산출물 `construction/novelty-agent-v2/`.
...
> **개정 (2026-07-28) — U11 문헌탐색·근거형성 v2 재설계(자율 에이전트) 편입**: 질문 게이트 `requirement-verification-questions-evidence-agent-v2.md`(Q1~Q15 확정)를 반영한다. **오케스트레이션 교체**: 고정 파이프라인(검색 1회 → 추출 1회) → **검색 전략만 자율인 도구 호출 루프**(FR-37 개정 — 질의 설계·깊이 판단·종료 판단은 LLM, 문장 추출·근거표 조립·날조 검사 게이트는 기계식 유지). 규칙 기반 의도 분류기는 폐기하고 질의 설계가 루프 판단으로 흡수된다(FR-36 개정). 진행 표시는 거시 상태 소수 + 결정 트레이스 파생 활동 피드(FR-36 개정). **근거 범위 확장 3건**: (1) 인용 대상을 **DocModel 전 블록 타입**(문단·리스트·알고리즘·표·그림·수식)으로 확대(FR-47 개정 — 종전 "표·그림·수식"), (2) 코퍼스 밖 논문은 **온디맨드 본문 취득 + DocModel 생성**을 거쳐 내부 논문과 동일하게 인용하고 색인은 백그라운드로 미룬다(FR-31 등급 체계 종결), (3) **그림은 온디맨드 비전 입력으로 읽어 근거로 사용**한다(멀티모달 카브아웃 개정 — 아래). 셋의 검증 강도 차이는 `SourceRef.sourceScope`(`fulltext|abstract|figure`)로 **출처 단위 표시**하고, `SourceRef.anchorType`으로 인용 객체 종류를 싣는다(D5 개정). **날조 검사 강화**: 인용문이 앵커가 가리키는 **바로 그 블록**에 있을 것을 요구하고(종전: 문서 어디든), 그림 유래 주장의 숫자는 그 논문 텍스트에 실재해야 한다. **범위 확장**: FR-45(루프 예산·반복 상한)·FR-46(결정 추적성)을 [U12] → **[U11/U12]**. **모듈·표면 정리**: 대화 표면을 `/api/evidence` 한 벌로 합치고 껍데기 `research` 모듈을 제거해 `research*` 이름을 ⑦ supervisor에 반납한다. **불변**: C-2 날조 금지·기계식 검증(LLM-judge 없음), 빈 성공 금지(INV-EV-2), U6 비용 단일 권위, DocModel 단일 writer=U1(U11은 요청만), `EvidenceFormationPort`/`SourceRef` 공유 계약 유지. 설계 산출물 `construction/evidence-agent-v2/`(기존 `u11-evidence-agent/`는 frozen 원본 보존).
...
| **FR-36** | **대화형 문헌탐색·근거형성 세션 [U11]** *(2026-06-29 신규 · 2026-07-28 v2 개정 — 질의 설계 자율화·진행 계약 교체)*: 로그인 필수 사용자가 채팅 형식으로 연구 질문을 입력하면 에이전트가 다논문 교차확인을 수행해 근거를 비교 정리한다. 하단 네비 전용 진입·세션 리스트·세션 내 멀티턴(이전 턴 근거·대상 논문 맥락 누적)을 지원한다. | 로그인 필수(SEC-8). 채팅 진입 시 문헌탐색·근거형성 모드 선택 가능. 후속 질문("그 중 한계가 가장 적은 건?")에 이전 맥락을 유지한 응답 반환. **후속 질문 해석·검색 질의 설계는 루프의 LLM 판단이며, 규칙 기반 의도 분류(정확 문구·꼬리질문 패턴 매칭)는 폐기한다**(v2). 진행 표시는 **거시 상태 소수**(접수 → 조사 중 → 정리 중 → 완료/부분/기권) + **결정 트레이스(FR-46) 파생 활동 피드**(현재 도구, 검색 질의 요약, 확인한 논문 수, 본문 취득 상태)로 구성한다(구 고정 단계 이벤트 대체). 하단 네비에서 전용 진입, 세션 리스트에서 기존 세션 재열람 가능. |
| **FR-37** | **다논문 근거형성 — 추출·비교·기권 [U11]** *(2026-06-29 신규 · 2026-07-28 v2 개정 — 자율 탐색 루프)*: **탐색은 자율 루프로 수행한다** — 어떤 질의로 검색하고, 어떤 논문을 깊이 읽고, 언제 충분한지를 LLM이 매 턴 판단하며(고정 단계 순서 없음), **문장 추출·근거표 조립·날조 검사 게이트는 기계식으로 유지한다**(C-2 fail-closed 불변, LLM-judge 없음). 탐색 대상은 내부 코퍼스와 **외부 검색 결과**를 모두 포함하며, 코퍼스 밖 논문은 온디맨드 본문 취득을 거쳐 근거가 된다(FR-31 등급 체계). 사용자 질문·명시 paper 집합·첨부 문서를 대상으로 다논문 교차확인 후 **핵심 주장·방법·결과 수치·한계**를 추출(C-2 경계: 논문 원문 기반 추출만, 생성 산문 금지)하고 **논문 비교형 + 쟁점 오버레이**(일치/상충 표시)로 정리한다. 근거 없으면 날조 대신 **기권**(FR-5). 결과는 `EvidenceItem{ statement, supporting[], conflicting[] }` 계약으로 반환하며 연구아이디어 유닛(U12)이 Tool로 소비한다(D5). | scope=auto(자동 검색)·explicit(명시 paper 집합)·mixed(혼합) 지원. 첨부는 doc-model 파이프라인 재사용(형식/크기 한도는 FD 이월). 코퍼스는 owned arXiv AI/ML 인덱스만(v1). confidence 제외(FR-5 그라운딩·환각 위험). conflicting[] 비어 있어도 성공 응답 가능. 근거 0건·범위 밖: state=abstain + 비기술 abstainReason(SEC-9, 내부 위반 상세 비노출). 긴 다논문 분석은 비동기 잡 오프로드 + 진행 상태 폴링(NFR-P6). 계약 SSOT: `shared/dtos/evidence.schema.json`. *(v2 추가)* 루프는 FR-45의 3중 한도 안에서만 동작하고, 한도 소진·부분 실패로 탐색이 완결되지 않으면 **검증된 근거는 그대로 반환하되 확인 범위**(확인한 논문 수 / 후보 수 + 중단 사유)를 `EvidenceCoverage`에 실어 사용자에게 표시한다 — 터미널 상태는 `ok|abstain` 2종을 유지한다(D5 union 불변). 사용자 문구에는 내부 용어(`degraded` 등)를 쓰지 않는다. |
...
| **FR-30** | **novelty Agent 입력·오케스트레이션 [U12]** *(2026-07-18 v2 개정)*: 자연어 연구 주제와 업로드 원고(PDF/Markdown/TXT)를 입력으로 받아 **완전 자율 도구 호출 루프**(LLM이 매 턴 다음 도구·질의·종료를 판단)로 조사를 수행한다. 문헌탐색·근거형성 Agent와는 별도 유닛이며, novelty Agent는 upstream `EvidenceFormationPort` 결과와 `SourceRef`만 소비한다. | 자연어 입력은 `form_evidence`를 선행 호출한다(근거 위에서만 조사 출발). 루프 종료 조건은 필수 산출물 완성(FR-32) 또는 예산 소진(FR-45)이며, 산출물 저장은 결정론 검증 게이트를 통과해야 한다. 업로드 문서는 공통 ingestion/doc-model 파이프라인 또는 문헌탐색·근거형성 Agent가 파싱하고, novelty Agent가 직접 파서/근거형성 로직을 재구현하지 않는다. DOCX는 v1 제외 유지. |
| **FR-31** | **novelty 검색·외부 탐색 [U12]** *(2026-07-18 v2 개정 — 메커니즘 중립화)*: U2 `full` 검색을 재사용해 내부 Corpus 유사 연구를 확장 검색하고, 외부 탐색은 특정 메커니즘에 고정하지 않는 **외부 탐색 어댑터**(일반 API 또는 MCP 서버)를 통해 수행한다. v1 범위는 GitHub 검색 + 관련 데이터셋 검색이며, 외부 arXiv 검색은 근거 등급 체계가 확정된 뒤 추가한다 *(2026-07-25 개정: 종전 "MCP 연동 단계에서 추가" — MCP 고정 단계 해제, `operations/solo-roadmap.md` 개정 블록)*. **등급 체계는 2026-07-28 문헌탐색 에이전트화 게이트에서 확정됐다**: 근거 범위는 `SourceRef.sourceScope` 3단(`fulltext`·`abstract`·`figure`)이며, 코퍼스 밖 논문은 온디맨드로 본문을 취득해 DocModel을 만든 뒤 내부 논문과 동일하게 인용하고(취득 실패·비OA는 `abstract` 범위로 인용), 색인은 백그라운드로 미룬다. DocModel 생성의 단일 writer는 U1이며 소비 유닛은 요청만 한다. 이 체계 위에서 `arxiv_search`가 합류할 수 있다. | EvidenceFormationPort 결과를 1차 근거로 삼고 U2 `full` 검색은 누락 보강/유사 논문 확장에 사용한다. novelty 전용 인덱스나 별도 랭킹 로직을 만들지 않는다. GitHub는 구현체·baseline·benchmark·license 단서만 추출하고 품질 점수/재현 가능 판정은 하지 않는다. 데이터셋은 이름·URL·라이선스·접근성·태스크·metric 후보를 산출한다. 외부로 나가는 도구는 도구별 payload allowlist를 어댑터가 기계식으로 강제한다(BR-NV6 승계). 뉴스 검색은 제외 유지. |
| **FR-32** | **조사 보고 + 여백 분석 [U12]** *(2026-07-18 v2 개정 — 기본 산출물 재정의)*: 기존 유사 연구를 표로 정리하고, **여백 분석**(주제 주변에서 무엇이 이미 연구됐고 어디가 비어 있는지의 판정)을 기본 산출물로 제공한다. | 유사 논문별 문제정의·방법·데이터셋·결과·한계·겹치는 점·SourceRef를 제공한다. 여백 분석의 모든 판정 항목에는 근거 링크(SourceRef)가 필수다. **방향 제안(bounded 차별화 후보)은 기본 산출물이 아니라 사용자가 대화로 요청할 때 온디맨드로 생성**하며, 기존 연구 한계와 코드/데이터셋 근거 안의 bounded 제안으로 제한한다. "새로움 확정", novelty 점수, 논문화 가능성 판정은 하지 않는다. |
| **FR-33** | **실험 계획 작성 (온디맨드) [U12]** *(2026-07-18 v2 개정)*: 사용자가 대화로 요청한 방향에 대해 실험 계획을 온디맨드로 작성한다. | 실험 계획은 가설, 차별화 포인트, baseline, 데이터셋, metric, 실험 절차, 리스크, 필요한 구현/리소스, 근거 링크를 포함한다. 기본 잡 산출물이 아니므로 요청이 없으면 생성하지 않는다. 온디맨드 산출물도 기본 산출물과 같은 결정론 검증 게이트를 통과해야 저장된다. 코드 skeleton이나 실행 스크립트 생성은 범위 밖 유지. |
| **FR-34** | ~~원고 위험 신호 [U12]~~ *(2026-07-18 v2 폐기)*: 원고 위험 신호(내부 Corpus 대비 문장/문단 유사도·AI 어투 경고)는 v2 임무(조사+여백 분석)와 결합이 약한 별개 기능으로 **v2 범위에서 제외**한다. | 필요가 확인되면 독립 기능으로 재도입한다(번호 재사용 방지를 위해 행 유지). v1 구현·frozen 설계(`construction/novelty-agent/`)는 기준선 기록으로 보존. |
| **FR-35** | **novelty Agent 진행상태·저장·Notion export [U12]** *(2026-07-18 v2 개정 — 진행 계약 교체)*: 에이전트 진행을 프론트에 표시하고, 결과를 owner-scoped로 내부 저장한 뒤 사용자 승인 시 Notion으로 export한다. | 진행 표시는 **거시 상태 소수**(접수 → 조사 중 → 보고 작성 중 → 완료/부분 완료/실패/취소) + **결정 트레이스(FR-46) 파생 활동 피드**(현재 도구, 검색 질의 요약, 발견 수, 실패/저하 사유)로 구성하며 스트리밍 또는 폴링으로 갱신한다(구 11종 enum 대체). 내부 DB/S3에 세션·입력 참조·트레이스·부분/최종 산출물·export 상태를 저장하고 삭제/초기화를 제공한다. Notion은 사용자별 OAuth/명시 연결, 토큰 암호화 저장, **미리보기 후 승인 export**(에이전트 자율 export 금지), 연결 해제/삭제를 제공한다. |
...
| **FR-44** | **novelty 세션 메모리 [U12]** *(2026-07-18 신규)*: 실행 중·완료 후 사용자 채팅이 조사를 스티어링(추가 지시·방향 수정)하고 온디맨드 산출물(FR-32/33)을 요청할 수 있는 **잡 내 멀티턴**을 제공한다. | 잡 내 대화가 1순위 도입 대상이며 기존 `NoveltyChatMessage` 모델을 저장 모델로 승계한다. 잡 간 사용자 메모리(새 잡이 이전 잡 산출물·선호 참조)는 목표 아키텍처에 정의하되 후속 도입한다. 대화 이력도 owner-scoped 저장·삭제 대상이다. |
| **FR-45** | **루프 예산·반복 상한 [U11/U12]** *(2026-07-18 신규 — 자율 종료 도입에 따른 안전 요건)*: 자율 루프는 3중 한도(최대 반복 수, 도구 호출 수[도구별 상한 포함], 토큰/비용 한도) 안에서만 동작한다. | 비용 판단은 U6 `get_budget_state()` 단일 권위를 유지하며 novelty 전용 CostGuard를 만들지 않는다. 한도 소진 시 그때까지 검증된 산출물만으로 `degraded`(부분 완료) 종료한다. 구체 수치는 NFR Requirements에서 확정한다. *(2026-07-28 범위 확장 [U12]→[U11/U12])* U11에도 동일 형태로 적용하며, **온디맨드 본문 취득·임베딩 비용도 이 한도에 계상**한다. U11의 한도 소진 종료는 검증된 근거 + 확인 범위 표기로 표면화된다(FR-37). 수치는 유닛별 NFR에서 각자 확정한다. |
| **FR-46** | **결정 추적성 [U11/U12]** *(2026-07-18 신규)*: 루프의 결정 트레이스(선택한 도구, 도구 인자·질의 요약, 결과 요약, 종료·저하 사유)를 잡에 귀속된 구조화 레코드로 저장한다. | 트레이스는 진행 표시 활동 피드(FR-35)의 원천이자 감사·디버깅·근거 재현성의 근거다. 루프 도입 시점부터 수집한다(소급 수집 불가). FE 노출 수준은 설계에서 정하되, 내부 원문 payload·자격증명·민감 로그는 노출하지 않는다(SEC-9/15). owner-scoped 저장·삭제 대상이다. *(2026-07-28 범위 확장 [U12]→[U11/U12])* U11 루프도 동일 형태의 트레이스를 저장하며, 이는 진행 활동 피드(FR-36)의 원천이자 근거 재현성의 근거다. |
| **FR-47** | **근거 대상 확장 — DocModel 전 객체 [U11/U12]** *(2026-07-18 신규 · 2026-07-28 개정 — 전 블록 타입·근거 범위 등급)*: 근거 검색·인용·앵커(`SourceRef`)는 문장뿐 아니라 **DocModel의 전 블록 타입**(`paragraph`·`list`·`code`(알고리즘)·`table`·`figure`·`formula`)을 가리킬 수 있어야 한다. `SourceRef`는 인용 객체의 종류(`anchorType`)와 **근거 범위**(`sourceScope`: `fulltext` 원문 인용 / `abstract` 초록 범위 / `figure` 그림 해석)를 함께 싣는다. | *(2026-07-28)* 종전 열거("표·그림·수식")를 전 블록 타입으로 넓힌다 — 여섯 타입 모두 결정적 블록 id와 `fullText` 투영을 이미 갖고 있어 검증 규칙만 타입별로 정의하면 된다. **검증 강화**: 인용문은 앵커가 가리키는 **바로 그 블록의 투영 안에** 있어야 한다(종전: 문서 어디든 존재하면 통과). **그림 근거**: 그림에서 얻은 주장의 숫자는 그 논문 텍스트(캡션·본문·표)에 실재해야 하며, 기계 검증이 불가능한 정성 서술(추세·구조)은 차단하지 않고 `sourceScope=figure`로 표시해 검증 강도 차이를 사용자에게 드러낸다. 앵커 실재성 검증은 결정론 유지(해당 객체가 그 논문 DocModel에 실재하는지). 공유 계약(`evidence.schema.json`의 SourceRef)과 문헌탐색 엔진(U11)에 걸친 변경이며, 구체 설계·도입 시점은 **문헌탐색 에이전트화 게이트에서 확정됨**(2026-07-28, `requirement-verification-questions-evidence-agent-v2.md` Q1~Q4). |
...
- **NFR-P5 [U12]** novelty Agent는 온디맨드 비동기 작업으로 처리한다. API는 job 생성/상태 조회/취소/잡 내 대화만 담당하고, 별도 Agent Worker가 자율 루프(U2 `full` 검색, 외부 탐색 어댑터, LLM, figure 조회)를 실행한다 *(2026-07-18 v2 개정: Agent-Browser → 외부 탐색 어댑터, 워커 = 루프 실행 주체)*. 검색 SLA(NFR-P1) 대상이 아니며, 재접속 후 상태 조회·취소·부분 결과 표시를 제공한다. 취소는 협조적(진행 중 도구 호출 완료 후 탈출, 검증·저장된 산출물 유지)이다.
...
- **NFR-C1** **월 비용 상한(hard cap) (제안: $300/월)**. **준실시간 지출/사용량 텔레메트리 + 임계 경보(제안: 80% 도달 시 경보)** 로 상한 초과 **이전에** 우아하게 저하하는 서킷 브레이커(제안: 100% 도달 전 LLM 리랭킹 비활성화 → lexical 검색 폴백). 이 텔레메트리/경보가 **RES-11(a) 비용 폭발 탐지 신호**를 제공한다(월 단위 청구가 아닌 인트라데이 폭주 포착). 이전 사이클의 CostGuard 패턴 계승. **[U1 Corpus 보강]** eager DocModel·GROBID·임베딩·OpenSearch/S3 저장은 사용량 비례가 아니라 코퍼스 전량 비용이므로, phase-1은 **최근 AI/ML 1년·OA/인덱싱 허용 라이선스·명시적 빌드 예산**으로 제한하고, 비용 임계치 도달 시 수집 우선순위(최신성/인용·팀 지정 seed) 밖 작업은 보류·DLQ/백필 큐로 이월한다. GROBID/임베딩/OpenSearch bulk/S3 저장 사용량은 U1 별도 비용 라인으로 계상한다. **[U7 보강]** 요약(FR-12)은 Sonnet 호출로 비용표에 없던 신규 라인(건당 ≈$0.1~0.2)이나, 온디맨드+영구저장으로 "distinct 논문×1회"만 과금되어 bounded → **기존 $1,600 상한 내 흡수**하되 U7 비용을 텔레메트리 **별도 라인으로 계상**하고 U6 CostGuard 게이트(예산 초과 시 요약 일시 기권, FR-11) 적용. **[U8 보강]** citation API는 U6 레이트리밋/CostGuard 신호와 U8 전용 쿼터 카운터를 사용하며, 임계 초과 시 캐시만 제공하거나 일시 기권한다. Infrastructure Design 비용표에 U1/U7/U8 라인 추가.
  - **[novelty Agent 보강]** U6 CostGuard와 rate limit을 재사용하고, U2 `full` 검색·LLM·tool call·외부 탐색 어댑터·Notion에 per-job budget을 둔다 *(2026-07-18 v2: FR-45의 3중 한도[반복·도구 호출·토큰/비용]로 구체화)*. 초과 시 전체 실패가 아니라 부분 결과와 `degraded` 상태를 반환한다. novelty Agent 전용 CostGuard는 만들지 않는다.
...
- **NFR-R3 [U12]** novelty Agent 외부 의존성(U2 `full`, EvidenceFormationPort, 외부 탐색 어댑터[GitHub·데이터셋·향후 arXiv], Notion, LLM)은 source별 실패/저하를 분리한다 *(2026-07-18 v2 개정: Agent-Browser → 외부 탐색 어댑터)*. 한 source 실패가 전체 job 실패로 전파되지 않으며, 성공한 source만으로 부분 산출물을 제공한다. 루프에서는 도구 실패가 오류 결과로 에이전트에 반환되어 대체 경로 판단 기회를 가진 뒤, 지속 실패 시 해당 source를 저하로 기록한다.
...
- **QT-10 — novelty Agent 품질/불변식 [U12]** *(2026-07-18 v2 개정)*: `EvidenceFormationPort`/`SourceRef` 무결성, U2 `full` 검색 결과의 SourceRef 정합, 유사 논문 적중, GitHub/데이터셋 출처 보존, **여백 분석 항목별 근거 링크 실재성, 산출물 저장 게이트(결정론 검증) 차단성, 루프 예산 상한 준수(FR-45), 결정 트레이스 무결성(FR-46)**, 실험 계획 필수 필드(온디맨드 경로), 거시 상태 전이 유효성, Notion export 무결성을 자동 테스트한다. 원고 위험 신호 항목은 FR-34 폐기로 제외. PBT는 Partial 적용(Q32=B)으로 pure function, DTO round-trip, source normalization, 상태/예산 invariant에 한정한다.
...
9. novelty Agent는 자연어 또는 업로드 원고 입력에서 공유 Evidence 계약과 U2 `full` 검색을 사용해 자율 루프로 **유사 연구 표와 여백 분석**을 생성하고, 사용자가 대화로 요청하면 방향 제안·실험 계획을 온디맨드로 추가 생성하며, 진행을 거시 상태 + 활동 피드로 표시하고, Notion export는 사용자 승인 후에만 수행한다. 외부 source 실패는 `degraded` 부분 결과로, 예산 소진은 검증된 산출물만의 부분 완료로 표면화된다. *(2026-07-18 v2 개정)* (FR-30..33·35, FR-44~47, NFR-P5/R3, QT-10)
10. 사용자가 연구 질문을 채팅으로 입력하면 에이전트가 **스스로 질의를 설계하며 내부 코퍼스와 외부를 탐색**해 관련 논문들에서 **핵심 주장·방법·결과·한계를 추출·비교 정리**해 받고, 근거가 없으면 날조 없이 기권한다. 근거는 DocModel 전 블록 타입(표 수치·수식·알고리즘·그림 포함)을 가리킬 수 있고, 각 출처의 근거 범위(전문/초록/그림 해석)가 표시된다. 탐색이 완결되지 않으면 확인 범위가 함께 표시된다. 세션이 owner-scoped로 영속되어 재열람 가능하다. *(2026-07-28 v2 개정)* (FR-36~38, FR-45~47, NFR-P6, QT-8)
...
> **[멀티모달 카브아웃 개정 — 2026-07-27]** 위 카브아웃(2026-06-22)의 "비전 LLM 추론 제외"를 **표시 경로에 한정**한다. FR-17이 D5로 예약해둔 "**차기 에이전트 on-demand 비전 입력**"이 유지보수 로드맵 ⑤3에서 발동했다: U12 novelty 에이전트가 `view_figure` 도구로 조회한 figure·수식 crop을 멀티모달 입력으로 받는다(BLM §3, BR-RA11 — DocModel에 실재하는 자산만, 온디맨드, 호출·토큰 비용은 루프 예산에 계상). **여전히 제외**: 요약·번역(U7)과 근거형성(U11)의 비전 추론, 그리고 그림·표·수식을 `SourceRef` 앵커로 인용하는 것(FR-47 — 공유 계약·U11 몫, 로드맵 ⑥ 게이트에서 확정). 즉 ⑤3은 "이미지를 에이전트 눈에 넣는 것"까지이고, 그것을 근거로 **인용**하는 것은 아니다.

> **[멀티모달 카브아웃 재개정 — 2026-07-28]** 위 2026-07-27 블록이 "여전히 제외"로 남겨둔 두 항목이 유지보수 로드맵 ⑥ 게이트에서 **모두 해제**됐다(`requirement-verification-questions-evidence-agent-v2.md` Q1~Q4): (1) **근거형성(U11)의 비전 추론** — 근거형성 에이전트가 `view_figure`로 그림·수식 crop을 온디맨드 조회해 멀티모달 입력으로 받는다, (2) **그림·표·수식을 `SourceRef` 앵커로 인용하는 것**(FR-47) — 인용 대상이 DocModel 전 블록 타입으로 넓어졌다. 그림 유래 근거는 `sourceScope=figure`로 표시하고, 그 주장의 숫자는 논문 텍스트에 실재해야 한다는 기계식 규칙을 받는다. **여전히 제외**: 요약·번역(U7)의 비전 추론. 자산 리더는 비전 입력용(바이트 반환)만 `shared/`로 공용화하고 U7의 표시용 리더(서명 URL)는 목적이 달라 합치지 않는다.
...
| FR-30~35 v2 개정 + FR-44~47 신규 (자율 루프·조사+여백 분석·온디맨드 산출물·위험 신호 폐기·세션 메모리·예산 상한·결정 추적성·근거 대상 확장) [U12/U11] | 아키텍처 재설계 Q1~Q8 + 기능 정의 Q1~Q7 + FD 게이트 Q1~Q14 확정 (2026-07-18); `requirement-verification-questions-agent-rearchitecture.md`; `requirement-verification-questions-novelty-v2-function.md`; `construction/plans/novelty-agent-v2-functional-design-plan.md`; 설계 `construction/novelty-agent-v2/` |
| FR-36~37 v2 개정 + FR-47 개정 + FR-31 등급 종결 + FR-45/46 범위 확장 (자율 탐색 루프·전 블록 타입 인용·온디맨드 본문 취득·그림 비전 근거·표면 일원화) [U11/U12] | 문헌탐색 에이전트화 게이트 Q1~Q15 확정 (2026-07-28); `requirement-verification-questions-evidence-agent-v2.md`; 상속 `requirement-verification-questions-agent-rearchitecture.md` Q2/Q4=A; 설계 `construction/evidence-agent-v2/` |
