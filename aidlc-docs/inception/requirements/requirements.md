...
> **개정 (2026-07-28) — U11 문헌탐색·근거형성 v2 재설계(자율 에이전트) 편입**: 질문 게이트 `requirement-verification-questions-evidence-agent-v2.md`(Q1~Q15 확정)를 반영한다. **오케스트레이션 교체**: 고정 파이프라인(검색 1회 → 추출 1회) → **검색 전략만 자율인 도구 호출 루프**(FR-37 개정 — 질의 설계·깊이 판단·종료 판단은 LLM, 문장 추출·근거표 조립·날조 검사 게이트는 기계식 유지). 규칙 기반 의도 분류기는 폐기하고 질의 설계가 루프 판단으로 흡수된다(FR-36 개정). 진행 표시는 거시 상태 소수 + 결정 트레이스 파생 활동 피드(FR-36 개정). **근거 범위 확장 3건**: (1) 인용 대상을 **DocModel 전 블록 타입**(문단·리스트·알고리즘·표·그림·수식)으로 확대(FR-47 개정 — 종전 "표·그림·수식"), (2) 코퍼스 밖 논문은 **온디맨드 본문 취득 + DocModel 생성**을 거쳐 내부 논문과 동일하게 인용하고 색인은 백그라운드로 미룬다(FR-31 등급 체계 종결), (3) **그림은 온디맨드 비전 입력으로 읽어 근거로 사용**한다(멀티모달 카브아웃 개정 — 아래). 셋의 검증 강도 차이는 `SourceRef.sourceScope`(`fulltext|abstract|figure`)로 **출처 단위 표시**하고, `SourceRef.anchorType`으로 인용 객체 종류를 싣는다(D5 개정). **날조 검사 강화**: 인용문이 앵커가 가리키는 **바로 그 블록**에 있을 것을 요구하고(종전: 문서 어디든), 그림 유래 주장의 숫자는 그 논문 텍스트에 실재해야 한다. **범위 확장**: FR-45(루프 예산·반복 상한)·FR-46(결정 추적성)을 [U12] → **[U11/U12]**. **모듈·표면 정리**: 대화 표면을 `/api/evidence` 한 벌로 합치고 껍데기 `research` 모듈을 제거한다. **불변**: C-2 날조 금지·기계식 검증(LLM-judge 없음), 빈 성공 금지(INV-EV-2), U6 비용 단일 권위, DocModel 단일 writer=U1(U11은 요청만), `EvidenceFormationPort`/`SourceRef` 공유 계약 유지. 설계 산출물 `solo-agent/evidence-agent-v2/`(기존 `u11-evidence-agent/`는 frozen 원본 보존).

> **개정 (2026-08-13) — 코퍼스 범위·배포 규모 확정 (재인셉션 페이즈 7 / 코퍼스·배포)**: 질문지 `requirement-verification-questions-corpus-and-deployment.md`(Q1~Q9 전부 확정)를 반영한다.
>
> **⚠️ 원문 보존 원칙**: 본 개정은 팀 프로젝트 시절에 확정된 조항의 **값을 덮어쓰지 않는다.** 아래 변경은 **2~3개월 한시 배포(개인 계정·프리티어)** 에 한정해 적용되며, 원 조항은 실서비스 기준 설계 기록으로 그대로 둔다. 해당 조항에는 본 블록을 가리키는 표시만 단다.
>
> **비용 (NFR-C1 관련 — 조항 원문 불변)**: 팀 AWS 크레딧을 전제한 상한($1,600/월 — `infrastructure-design.md` §0, 본 문서 NFR-C1의 제안치 $300/월)은 이번 배포에 적용되지 않는다. **이번 배포 예산 = 신규 개인 계정 프리티어 크레딧(최대 $200, 6개월 또는 소진 시까지) 중 DocSuri 몫 3개월 ~~$72~~ → $144** (Lightsail ~~`medium_3_0` 4 GB~~ → **`large_3_0` 8 GB** 단일 인스턴스 — 2026-08-15 개정, 근거는 코퍼스·배포 질문지 Q1: 편당 1.73 MB 실측으로 4 GB의 실제 천장이 약 1,750편이라 최근분 자리가 없다). 같은 계정을 puppytalk과 공유하며 Lightsail 무료 번들(계정당 1개, 최대 2 GB)은 puppytalk이 사용한다. 비용 통제는 월 상한이 아니라 **총액 + AWS Budgets 알람**으로 한다.
>
> **인프라 (조항 신설 없음 — Infrastructure Design 사안)**: 배포 토폴로지는 **Lightsail 단일 인스턴스**(로컬 `docker compose` 구성을 그대로 이관, 앱은 어댑터 배선만 교체)로 한다. `ops/cdk/` 스택 9종(VPC·ECS Fargate 6·ALB·RDS·OpenSearch Domain·ElastiCache·SQS 14 등 — NAT Gateway는 원 설계에서 이미 배제, `infrastructure-design.md` §0)은 **실서비스 토폴로지 설계 기록으로 보존**하며 이번 배포에 사용하지 않는다.
>
> **코퍼스 범위 (FR-6·NFR-C1 [U1 Corpus 보강] 관련 — 조항 원문 불변)**: (구) phase-1 범위 "최근 AI/ML 1년"(u1-corpus Q12=A)을 이번 배포용으로 재정의한다. **편수 천장 약 4,500편**(배포 스택 앱 메모리 실측 250 MB 기반 산정, 청크·차원 완화책 미적용) = **기반 논문 약 1,500편(arXiv ID 목록 지정 수집) + 최근분 약 3,000편(AI/ML 날짜 창)**. 기반 논문분을 먼저 확보한다 — **날짜 창 수집만으로는 기반 논문이 구조적으로 들어오지 않는다**(실측: 로컬 코퍼스 29,915편에서 대표 기반 논문 10편 중 1편만 존재. 날짜 창에는 과거 논문이 "최근 개정판이 올라와 걸린 것"만 우연히 포함되기 때문). 배포 코퍼스는 **전량 신규 수집·파싱**하며 로컬 개발 코퍼스를 재사용하지 않는다. 배포 기간 중 **증분 수집은 하지 않는다**(스냅샷 고정 — 파싱은 로컬, 배포에는 색인 결과만 올린다).
>
> **소스 적재 (FR-6 관련)**: arXiv에 더해 **S2/OpenAlex도 코퍼스에 적재**한다. 단 ⑧-1.7 실측(OpenAlex PDF의 약 50%가 출판사 403 · OpenAlex에 분야 필터 부재로 타 분야 대량 유입 · S2를 OR로 넓히면 약탈적·소규모 저널 다수)을 상쇄하기 위해 **적재 전 분야 필터**와 **저널 품질 하한**을 선행 구현한다(U1 작업 항목).
>
> **신규 등재 — FR-48(커버리지 한계 노출 및 판정 보류) [U11/U12]**: 아래 FR 표 참조. 코퍼스가 유한하다는 사실을 제품이 드러내고, U12는 색인 범위 밖 대상에 대해 판정을 보류한다. **본 개정이 확정하는 것은 "보류 경로를 만든다"까지이며, 무엇을 범위 밖으로 볼지의 판정 조건은 U12 기능 설계 게이트로 이월한다** — 후보 3종(검색 점수·건수 임계 / 주제 분류기 / LLM 판단)이 각각 정량 근거 부재·신규 컴포넌트 필요·C-2 결정론 충돌 문제를 가져 이 단계에서 임계값을 정할 근거가 없다.
>
> 영향 유닛: U1(`ingestion/`) · U2(색인 소비) · U11 · U12 · 배포 인프라.
...
| **FR-3** | 관련도순으로 정렬해 상위 N건을 빠르게 반환한다. *(2026-07-06 개정 — 페이즈 7)* 융합 상위 M 후보를 cross-encoder 재랭킹으로 재정렬해 순서 품질을 높인다. | 상위 N건(제안: 20) 반환, 문서화된 관련도 점수순 정렬. **재랭킹**: 예산 저하/어댑터 실패 시 융합점수(RRF) 순서로 fail-soft(응답 비차단). 정량 품질 평가(nDCG/MRR/라벨셋)는 클릭로그·라벨 데이터 확보 후 수행; 현 단계=기능 검증 + 대표 질의 정성 비교. M=100(lite)/150(full) — 2026-08-15 상향(근거=tail 폐기 손실 복구, 정량 평가 아님). |
...
| **FR-48** | **커버리지 한계 노출 및 판정 보류 [U11/U12]** *(2026-08-13 신규 — 코퍼스·배포 질문지 Q8=A)*: 코퍼스가 유한하다는 사실을 제품이 드러내고, U12는 **색인 범위 밖 대상에 대해 novelty 판정을 보류**한다. 두 부분으로 구성된다 — (1) 색인 범위 상시 표시(검색·에이전트 진입 등 고정 위치), (2) **판정 보류 응답 경로**(범위 밖으로 판정되면 "새롭다/아니다"를 내지 않는다). | 근거: U12는 "선행연구를 못 찾음"과 "선행연구가 없음"을 구분할 수단이 없어, 범위 밖 질의에 대해 **전 건 "새롭다"로 응답**하며 그 출력은 정상 출력과 형태가 같아 기계 검사로 걸러낼 수 없다(제품 최대 오답 경로). (1)만으로는 방어되지 않는다 — 응답에 "새로운 접근으로 보입니다"가 뜨면 고정 안내문은 읽히지 않으므로 **실질은 (2)** 다. **범위 밖 판정 조건은 본 FR이 정하지 않고 U12 기능 설계 게이트로 이월**한다(후보: 검색 점수·건수 임계 / 주제 분류기 / LLM 판단 — 각각 정량 평가셋 부재·신규 컴포넌트 필요·C-2 결정론 충돌). 보류 응답은 기권(FR-5·INV-EV-2)과 같은 계열로 다루되 사유를 구분해 표기한다. |
...
| FR-30~35 v2 개정 + FR-44~47 신규 (자율 루프·조사+여백 분석·온디맨드 산출물·위험 신호 폐기·세션 메모리·예산 상한·결정 추적성·근거 대상 확장) [U12/U11] | 아키텍처 재설계 Q1~Q8 + 기능 정의 Q1~Q7 + FD 게이트 Q1~Q14 확정 (2026-07-18); `requirement-verification-questions-agent-rearchitecture.md`; `requirement-verification-questions-novelty-v2-function.md`; `solo-agent/plans/novelty-agent-v2-functional-design-plan.md`; 설계 `solo-agent/novelty-agent-v2/` |
| FR-36~37 v2 개정 + FR-47 개정 + FR-31 등급 종결 + FR-45/46 범위 확장 (자율 탐색 루프·전 블록 타입 인용·온디맨드 본문 취득·그림 비전 근거·표면 일원화) [U11/U12] | 문헌탐색 에이전트화 게이트 Q1~Q15 확정 (2026-07-28); `requirement-verification-questions-evidence-agent-v2.md`; 상속 `requirement-verification-questions-agent-rearchitecture.md` Q2/Q4=A; 설계 `solo-agent/evidence-agent-v2/` |
...
| FR-48 (커버리지 한계 노출·U12 판정 보류 — 조건은 기능 설계 이월) [U11/U12] | 코퍼스·배포 Q8=A (2026-08-13) |
| 코퍼스 범위 재정의 (4,500편 = 기반 논문 1,500 + 최근분 3,000, 전량 신규 수집) [U1] | 코퍼스·배포 Q1=B·Q4=C·Q6=B (2026-08-13) |
| S2/OpenAlex 적재 + 적재 전 분야 필터·저널 품질 하한 [U1] | 코퍼스·배포 Q3=B (2026-08-13); ⑧-1.7 실측 |
| 배포 예산·토폴로지 (Lightsail 4GB 단일 인스턴스 / 3개월 $72 — NFR-C1 원문 불변) | 코퍼스·배포 Q1=B·Q2=A (2026-08-13) |
| 증분 수집 없음 (스냅샷 고정 — 파싱 로컬, 배포엔 색인만) [U1] | 코퍼스·배포 Q7=B (2026-08-13) |
