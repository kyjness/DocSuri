...
| **FR-31** | **novelty 검색·외부 탐색 [U12]** *(2026-07-18 v2 개정 — 메커니즘 중립화)*: U2 `full` 검색을 재사용해 내부 Corpus 유사 연구를 확장 검색하고, 외부 탐색은 특정 메커니즘에 고정하지 않는 **외부 탐색 어댑터**(일반 API 또는 MCP 서버)를 통해 수행한다. v1 범위는 GitHub 검색 + 관련 데이터셋 검색이며, 외부 arXiv 검색은 근거 등급 체계가 확정된 뒤 추가한다 *(2026-07-25 개정: 종전 "MCP 연동 단계에서 추가" — MCP 고정 단계 해제, `operations/solo-roadmap.md` 개정 블록. 등급 체계는 FR-47과 함께 문헌탐색 에이전트화 게이트에서 확정)*. | EvidenceFormationPort 결과를 1차 근거로 삼고 U2 `full` 검색은 누락 보강/유사 논문 확장에 사용한다. novelty 전용 인덱스나 별도 랭킹 로직을 만들지 않는다. GitHub는 구현체·baseline·benchmark·license 단서만 추출하고 품질 점수/재현 가능 판정은 하지 않는다. 데이터셋은 이름·URL·라이선스·접근성·태스크·metric 후보를 산출한다. 외부로 나가는 도구는 도구별 payload allowlist를 어댑터가 기계식으로 강제한다(BR-NV6 승계). 뉴스 검색은 제외 유지. |
...
| **FR-45** | **novelty 루프 예산·반복 상한 [U12]** *(2026-07-18 신규 — 자율 종료 도입에 따른 안전 요건)*: 자율 루프는 3중 한도(최대 반복 수, 도구 호출 수[도구별 상한 포함], 토큰/비용 한도) 안에서만 동작한다. | 비용 판단은 U6 `get_budget_state()` 단일 권위를 유지하며 novelty 전용 CostGuard를 만들지 않는다. 한도 소진 시 그때까지 검증된 산출물만으로 `degraded`(부분 완료) 종료한다. 구체 수치는 NFR Requirements에서 확정한다. |
| **FR-46** | **novelty 결정 추적성 [U12]** *(2026-07-18 신규)*: 루프의 결정 트레이스(선택한 도구, 도구 인자·질의 요약, 결과 요약, 종료·저하 사유)를 잡에 귀속된 구조화 레코드로 저장한다. | 트레이스는 진행 표시 활동 피드(FR-35)의 원천이자 감사·디버깅·근거 재현성의 근거다. 루프 도입 시점부터 수집한다(소급 수집 불가). FE 노출 수준은 설계에서 정하되, 내부 원문 payload·자격증명·민감 로그는 노출하지 않는다(SEC-9/15). owner-scoped 저장·삭제 대상이다. |
| **FR-47** | **근거 대상 확장 — DocModel 전 객체 [U11/U12]** *(2026-07-18 신규)*: 근거 검색·인용·앵커(`SourceRef`)는 문장(텍스트 청크)뿐 아니라 DocModel의 **표·그림·수식 객체**까지 가리킬 수 있어야 한다. | 표에만 있는 수치·그림이 보여주는 구조를 근거로 잡을 수 있게 한다. 앵커 실재성 검증은 결정론 유지(해당 객체가 그 논문 DocModel에 실재하는지). 공유 계약(`evidence.schema.json`의 SourceRef)과 문헌탐색 엔진(U11)에 걸친 변경이므로 구체 설계·도입 시점은 문헌탐색 에이전트화(유지보수 로드맵 ⑥) 질문 게이트에서 확정한다. |
