...
>
> **⚠️ 원문 보존 원칙**: 본 개정은 팀 프로젝트 시절에 확정된 조항의 **값을 덮어쓰지 않는다.** 아래 변경은 **2~3개월 데모 배포(개인 계정·프리티어)** 에 한정해 적용되며, 원 조항은 실서비스 기준 설계 기록으로 그대로 둔다. 해당 조항에는 본 블록을 가리키는 표시만 단다.
>
