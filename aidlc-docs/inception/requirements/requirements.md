...
>
> **비용 (NFR-C1 관련 — 조항 원문 불변)**: 팀 AWS 크레딧을 전제한 상한($1,600/월 — `infrastructure-design.md` §0, 본 문서 NFR-C1의 제안치 $300/월)은 이번 배포에 적용되지 않는다. **이번 배포 예산 = 신규 개인 계정 프리티어 크레딧(최대 $200, 6개월 또는 소진 시까지) 중 DocSuri 몫 3개월 $72** (Lightsail `medium_3_0` 4 GB 단일 인스턴스). 같은 계정을 puppytalk과 공유하며 Lightsail 무료 번들(계정당 1개, 최대 2 GB)은 puppytalk이 사용한다. 비용 통제는 월 상한이 아니라 **총액 + AWS Budgets 알람**으로 한다.
>
