...
| 표 — 박스 붕괴로 매칭 실패 | GROBID 박스가 캡션 줄로 쪼그라들어 실제 표와 겹침 0 | ⬜ **미해결(아래)** |
...
**남은 표 결함 — 자동 수리를 열지 않은 이유**: `1909.03716`의 빈 표는 GROBID 박스가 높이 21pt의
캡션 줄(y 258~279)로 붕괴했고, 같은 페이지의 실제 표는 y 62~247과 y 291~393이라 겹침이 0이다.
위 11pt·아래 12pt로 거의 동률이어서 최근접 대체는 **엉뚱한 표의 숫자를 이 캡션 아래 붙일 수 있다**
— 페이지에 인쇄된 숫자만 쓰는 C-2 검증은 이 오귀속을 막지 못한다(숫자는 실재하되 블록이 틀린다).
해당 페이지는 GROBID 표 블록 1개 대 Docling 표 2개라 순서 매칭도 성립하지 않는다.
캡션 텍스트 대조 등 **기하학 밖의 신호**가 필요하며, 착수 여부는 별도 판단.
