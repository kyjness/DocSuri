...
## 게이트 레인 부재 (2026-07-20 확인)

u1 정리에서 `PostgresControlPlaneStore`가 "테스트 없음"이 아니라 **게이트 뒤에서 아무도 안 도는 상태**
였던 것이 드러나, u2·u7의 스킵 항목도 같은지 확인했다. 결과: **`ci.yml`에 `services:` 블록이 없어
게이트 테스트는 CI에서 하나도 실행되지 않는다.** 로컬 기본 실행과 동일하게 스킵된다.

| 스킵 | 유닛 | 필요 조건 | 상태 |
|---|---|---|---|
| `test_assets_rds_real.py` (2건) | u7 | `DOCSURI_TEST_PG_DSN` | CI에 Postgres 서비스 컨테이너 없음 |
| `test_integration_real.py` | u7 | boto3 + `DOCSURI_SUMMARY_BUCKET` | AWS 은퇴로 버킷 부재 — **②의 s3proxy를 보도록 재작성 대상** |
| `test_opensearch_integration.py` | u2 | `real` extra + `DOCSURI_OPENSEARCH_ENDPOINT` | CI가 `--extra api`만 설치(모듈 import 스킵), extra를 넣어도 엔드포인트 없어 재스킵 — **원인 2겹** |

셋 다 코드·테스트는 온전하며 실행 조건만 없다. u2 OpenSearch와 u7 S3는 ②단계 로컬 이관으로
**컨테이너가 이미 존재**하므로 대상이 AWS에서 로컬로 바뀐 것뿐이고, u7 Postgres는 CI에
서비스 컨테이너 한 건 추가로 해결된다. 지금 고치지 않는 이유는 ⑤a 축소와 같다(회귀 위험이지
현존 결함이 아님) — 각 유닛을 다시 만들 때 그 작업에 포함한다.
