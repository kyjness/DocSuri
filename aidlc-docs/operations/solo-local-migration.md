...
| 배포 | https://docsuri.org (Route53) | **보류** — 도메인 갱신만 유지, 서버 비용은 포트폴리오 시점에 재결정 |
...
| 결정 | 보류 사유 | 재검토 시점 |
|---|---|---|
| 배포 (docsuri.org 재연결) | 서버 비용. 도메인 갱신(연 ~$13 + 호스팅존 $0.5/월)만 유지 | 포트폴리오 제출 전. 후보: Lightsail(~$5/월) / 프론트만 Vercel / 데모 영상 대체 |
