...
- Current gate: ~~⑤3(멀티모달 `view_figure`)~~ → ✅ 완료(2026-07-27). 현재 게이트는 **⑥ 문헌탐색
  에이전트화** — **질문 게이트 확정(2026-07-28, 아래)**. 다음은 설계 세트 작성.
...
- **근거 범위 3축 확장**: (1) 인용 대상 = DocModel **전 블록 타입**(문단·리스트·알고리즘·표·그림·수식)
  — 여섯 타입 모두 블록 id와 `fullText` 투영을 이미 갖고 있어 검증 규칙만 타입별로 정의하면 된다.
  (2) 코퍼스 밖 논문 = **온디맨드 본문 취득 + DocModel 생성**(동기) 후 내부 논문과 동일하게 인용,
  **색인은 백그라운드**(인용에 필요 없는 단계라 답변을 막지 않는다). 취득 실패·비OA는 초록 범위로
  잔류하고 배제하지 않는다. (3) **그림 = 온디맨드 비전 입력으로 읽어 근거로 사용**.
- **검증 강화**: 인용문이 **앵커가 가리키는 그 블록 안에** 있을 것(종전: 문서 어디든 존재하면 통과).
  그림 유래 주장의 숫자는 논문 텍스트(캡션·본문·표)에 실재해야 한다. 기계 검증 불가한 정성 서술은
...
- **리뷰 3종 완료 (2026-08-01~02)** — `/simplify` 48건 중 33건 반영(커밋 379fb5e·5db9963),
  `/code-review` 확정 26건, `/security-review` 취약점 0건.
- **`/code-review` 회계** — 수리 16 / 사유 있는 스킵 9 / 조치 불가 1(리뷰가 상세를 보고하지
  않아 대상 특정 불가). 스킵 9건은 재감사에서 주장이 반박됐거나(3), 이미 처리돼 있거나(2),
  ⑦에서 하기로 확정된 공용화(3), 구조의 귀결(1)이다.
- **스킵 판정을 뒤집은 1건 — 게이트 숫자 표기 정규화.** 처음에는 "eval 없이 날조 게이트를
  완화하지 않는다"로 스킵했으나 **전제가 틀렸다**: u7 grounding이 같은 문제를 이미 풀어
  QT-1 eval 코퍼스(라벨 32건, 오통과 0 구간)로 검증해 뒀고, evidence만 정규화 없이 갈라져
  있던 **저장소 내 불일치**였다. u7 규칙 그대로 이식 — 등가 표기(정확 ×100/÷100)는 인정,
  반올림 허용은 **같은 스케일에서만**(스케일 교차는 정수 "20"↔연도 "2020" 반례 때문에 계속
  fail-closed). 교훈: 스킵 사유가 "선례가 없다"일 때는 **저장소 안에 선례가 있는지부터**
  본다.
- **`/security-review`** — 탐지 1건(임의 `paperIds`가 doc-model 저장소에 도달)을 관점이 다른
  검증 2대가 모두 반증(신뢰도 3·9). 남의 문서를 읽으려면 피해자 계정 UUID가 필요하고 그
  UUID는 어떤 응답으로도 노출되지 않으며, 그 외 도달 가능한 것은 기존 엔드포인트가 이미
  임의 id로 서빙하는 공개 코퍼스뿐이다. **새로 생긴 읽기 권한 없음.**
- **취약점은 아니지만 계약과 어긋나 고친 3건**(커밋 d6a6127·3c367af): `auto` scope에서도
  `paperIds`를 씨앗으로 올림(계약·BR-EV-2 위반) · 업로드 네임스페이스(`userdoc:`)가 코퍼스
  id로 위장 가능 · 첨부 신원을 클라이언트 값의 **자기정합**으로만 검증(인증된 owner_id에서
  재유도해 대조하도록 변경). 셋 다 "계정 id가 새는 순간" 사적 문서 읽기·덮어쓰기로 바뀌는
  결합이라 결합 자체를 끊었다.
- **동기 SSE 이중 실행 봉쇄**(커밋 57e244a): 동기 턴에는 jobId가 없어 스트림 단절이 "시작도
  못 함"과 구분되지 않았고, 클라이언트가 JSON으로 재전송해 같은 턴이 두 번 실행됐다(LLM 비용
  이중 지출). 서버가 턴 저장 직후 `accepted`로 복구 좌표를 흘리고, 클라이언트는 시작된 뒤의
  단절을 재전송 대신 스냅샷으로 복구한다.
- **머지 완료 (2026-08-02, PR #18 → develop `9b4b4d2`)** — 커밋 25개, 104파일.
- **머지 직전 로컬 스모크에서 결함 1건**: `scope=explicit`가 구조적으로 동작하지 않았다.
  지정 논문은 `state.discovered`에 씨앗으로 올라가는데 관찰은 `state.papers`만 렌더해,
  검색 도구가 없는 explicit에서 모델이 부를 id를 알 방법이 없어 지어냈다(`WJ-23-347` 등
  3회 → 캡 소진 → insufficient_evidence). auto에서는 검색 결과가 최근 도구 결과로 보여
  가려져 있었고, 루프 단위 테스트는 상태를 직접 조립해 넣으므로 "관찰에 무엇이 실리는가"를
  검사하지 않아 CI 14레인 전부 초록이었다. 관찰에 `pending_papers`를 추가해 수리 —
...
- 다음: 두 에이전트 완성 — ⑩(novelty 임무 축소) + ⑨(종료 판단·중간 취소). 별도 사이클,
  명시적 착수 지시 후.

## ⑤ 3단계 — 멀티모달 `view_figure`

- Date: 2026-07-27 · Branch: `feature/u12-view-figure`
- `view_figure`를 **2모드**로 등재(BLM §3.2 신설): `asset_id` 없이 호출하면 자산 목록(assetId·
...
- 첨부는 **decide 직후 소비되는 1회성**. 종전 배치(도구 실행 뒤 정리)는 종료 제안이 거부돼
  도구 없이 되도는 경로(`loop._drive`의 continue)를 놓쳐 같은 그림을 3회 재전송했다(회귀
  테스트로 확인). 관찰 조립 안에서 지우는 안도 기각 — 조회 이름의 함수가 숨은 변이를 갖게 돼
  재-decide가 첨부를 잃는다.
- requirements **멀티모달 카브아웃 개정**(2026-07-27): "비전 LLM 추론 제외"를 표시 경로에
  한정하고 U12 에이전트 on-demand 조회를 해제(FR-17 D5 발동). 요약·번역·근거형성의 비전 추론과
  FR-47(그림·표를 `SourceRef`로 인용)은 **여전히 제외** — 후자는 ⑥ 게이트 몫이다.
- 등록 조건은 자산 토글 + postgres. 버킷 env는 조건에서 뺐다 — `DOCSURI_S3_BUCKET`이 인제스천
  스택에만 있어 배포 환경에서 도구가 무증상으로 사라진다. 배포 스택(`novelty_stack`)에 자산
  토글과 `assets/*` GetObject를 추가(워커가 도구 레지스트리를 만드는 유일한 프로세스인데
  토글이 API 태스크에만 있었고 crop 경로 권한은 아예 없었다).
- **로컬 실스택 검증(2026-07-27)** — 잡 3건·대화 턴 5회. 확인된 것과 고친 것:
  - `view_figure` 조건부 등록이 실제로 동작(워커 로그의 도구 목록에 등장), 2모드·이미지
    첨부·S3 조회 전 경로 정상. **비전이 실제로 작동함을 육안 대조로 확정**: 모델이 묘사한
    패널 구성(3D 산점도 3종 + FOV 막대그래프)이 어떤 캡션에도 없는 내용이었고, 크롭 원본을
    직접 열어 일치를 확인했다.
  - 🔧 **`TURN_SYSTEM_PROMPT`에 view_figure 사용 규칙이 없었다** → 대화 턴에서 모델이
    "이미지를 열람할 수 없다"며 도구를 아예 호출하지 않음. 규칙 추가로 수리.
  - 🔧 **`max_turn_steps=4`가 2모드 계약에 비해 빡빡했다**(바닥이 목록·조회·답변 3스텝) →
    그림 두 장을 보면 답변 스텝이 없어 조회 성공 후에도 실패 안내가 나갔다. 6으로 상향
    (NFR-NV2-7 갱신).
  - 🔧 **도구 설명의 "꼭 필요한 것만"이 이미지 열기를 예외로 프레이밍**해 모델이 목록만
    받고 캡션으로 답했다 → "캡션은 그림이 무엇을 그렸는지 알려주지 않는다"로 재작성,
    재측정에서 이미지를 여는 것을 확인.
  - 🔧 **자율 루프가 view_figure를 스스로 쓰지 않았다(잡 2건 중 0회).** 원인은 프롬프트
    안의 경쟁이었다 — "이미지는 비싸므로 캡션만으로 충분하면 조회하지 않는다"가 바로
    다음 줄의 "남은 예산을 보고 우선순위를 정한다"와 맞물려, 모델이 언제나 조회를
    포기하는 쪽으로 해소했다. 브레이크를 걷어내고 필수 산출물에 묶인 트리거로 바꿨다
    (similar_works의 "이 논문이 무엇을 하는가"가 초록만으로 불분명할 때, gap_analysis의
    "이미 다뤄졌는가"가 갈릴 때). 재측정(잡 `d03d1519`, 25스텝): **0회 → 6회**
    — 목록 3회·이미지 3회. 캡 강제는 시스템 몫이라는 것도 명시해 모델이 아끼지 않게 했다.
  - 🔧 같은 재측정에서 드러난 **반복 호출** — 별건으로 분리해 수리했다(아래 §반복 호출).
    view_figure 6회 중 서로 다른 대상은 2개뿐이고(한 논문의 목록·`figure:0`),
    `corpus_search`는 거의 같은 질의로 13회 돌아 검색 캡 12/12를 소진했다.
  - ⚠️ u1 데이터 결함 관찰: `2106.16198:v1:figure:3`의 crop이 알고리즘·표·다른 그림을 함께
    담고 있고 캡션과 어긋난다(같은 논문 `figure:0`은 정합) — 체계적 오프셋이 아니라 개별
    crop 영역 결함. u1 몫([[u1-parse-completeness-audit]]).
  - 관찰: 매니페스트에 논문 자체의 그림 번호(`anchorLabel`)가 없어 모델이 "Figure 5"를
    assetId ordinal로 오라벨링한다. DocModel에만 있는 값이라 노출하려면 추가 읽기가 필요.
  - 비용: 잡 1건 $0.0154 / $0.50 한도 — 이미지 진입 후에도 여유.
- 리뷰 3종 반영: `/simplify`(첨부 소비 지점, 자산 조회 왕복 2회→1회, 죽은 코드 4종),
...
## ⑤ 3단계 후속 — 반복 호출: 관찰이 자기 질문을 보여주지 않았다

- Date: 2026-07-27
- Stage: CONSTRUCTION → Code (novelty-agent-v2)
- Trigger: ⑤3 재측정에서 루프가 **같은 인자로 반복 호출**하는 것이 드러났다. ⑤3이 만든
  결함이 아니라 이전 잡에도 있던 루프 공통 특성이라 별건으로 분리했다.
- Decisions:
  - **관찰의 결과 줄에 그 결과를 낳은 호출의 인자를 싣는다.** 종전에는 `[6] corpus_search
    (ok)` + 결과 JSON뿐이라, 최근 결과 6건이 매 턴 다시 실리는데도 모델은 그게 **어떤
    질의였는지** 알 수 없었다. 반복을 피할 근거 자체가 관찰에 없었던 것이다.
  - 값은 트레이스가 이미 갖고 있던 `summarize_args` 결과를 재사용한다. `execute_step`이
    한 걸음에 한 번 만들어 트레이스와 관찰에 같이 넘긴다.
  - 인자는 **모델이 쓴 값**이므로 오류 문구·assetId와 같은 `_flatten_line` 무해화를
    거친다 — 개행 한 줄로 없는 결과 줄이나 구획 경계를 지어낼 수 있어 예외를 두지 않는다.
  - 프롬프트에는 사용법만 덧붙였다: 이미 부른 인자를 그대로 다시 부르지 말 것, 검색이
    원하는 것을 주지 않았으면 표현을 바꾸지 말고 **다른 것을 물을 것**.
- 로컬 실스택 측정(같은 주제 3회, 기준선 1 + 수정 후 2):

  | | 기준선 `d03d1519` | `8b748b6f` | `bcf6a0bd` |
  |---|---|---|---|
  | 종료 | partial | **completed** | **completed** |
  | 스텝 | 25 | 13 | 6 |
  | corpus_search (서로 다름/전체) | 8/13 | **7/7** | **2/2** |
  | 캡 소진(`budget_denied`) | 1 | 0 | 0 |
  | 필수 산출물 | 2/3 (gap_analysis 결손) | **3/3** | **3/3** |
  | 비용 | $0.0311 | $0.0120 | $0.0034 |

  **반복은 낭비가 아니라 완주를 막고 있었다.** 검색 캡을 태운 뒤 gap_analysis를 만들지
  못한 채 partial로 끝나던 것이, 인자를 보여주자 두 번 다 완료로 바뀌었다.
- 남은 관찰(종결): 수정 후 두 잡 모두 `view_figure`를 **목록만 부르고 이미지는 열지
  않았다**(자산 5건·2건이 있었는데도). 프롬프트 규칙이 막은 것은 아니다 — 목록과 조회는
  인자가 달라 "같은 인자 반복" 대상이 아니고, 목록 뒤에도 6스텝이 남은 잡에서도 열지
  않았다. 루프가 헤매기를 멈추자 캡션으로 필수 산출물의 텍스트 필드를 채우기에 충분하다고
  판단하는 것으로 보인다. **비전 자체는 대화 턴 경로에서 실증돼 있다**(⑤3 검증: 어떤
  캡션에도 없는 패널 구성을 묘사). 이미지를 열게 하려고 프롬프트를 더 조이는 것은 열람
  자체를 목적으로 삼는 것이라 하지 않는다 — 그림이 필요한 판정을 요구하는 산출물이
  생기면(⑥ FR-47 그림 인용) 그때 자연히 필요해진다.

## ⑤ 2단계 후속 — 저장 게이트 계약을 모델이 알 수 있게

- Date: 2026-07-26
- Stage: CONSTRUCTION → Code (novelty-agent-v2)
- Trigger: ⑤2 실스택 검증 중 **에이전트가 잡을 한 번도 완료하지 못한다**는 사실이 드러났다.
  트레이스상 `save_artifact` 19회가 전부 `empty_artifact`로 거부됐다. 원인은 ⑤1(게이트)
  계약이 모델에게 보이지 않았던 것이라 ⑤2와 분리해 다뤘다.
- Decisions:
  - **거부 사유는 수리 가능해야 한다.** 툴 스펙의 payload가 스키마 없는 `{"type": "object"}`
    였으므로 모델은 컨테이너 키를 추측했고(`works`), 게이트는 `payload["items"]`만 읽어
    "비어 있다"로 거부했다. 그 사유는 **무엇이 틀렸는지를 담지 못해** 재시도가 수렴할 수
    없었다 — 결정론 게이트의 사유는 판정 결과이자 수리 지시다. 사유에 실제로 쓴 배열 키를
    담고, kind별 payload 형태를 스펙에 싣는다. 오답 키 이름을 미리 나열해두는 방식은 목록에
    없는 이름에서 다시 뭉뚱그려지므로 쓰지 않는다.
  - **데이터가 계약을 담는다.** `corpus_search` 카드가 값을 `arxivId`로만 보여주는데 게이트는
    `recordRef`를 요구해, 두 번째 겹의 `unknown_source_ref`가 났다. 카드가 게이트와 같은
    이름(`paperId`·`recordRef`)으로 값을 싣는다 — 프롬프트 설명에만 기대지 않는다.
  - **한도 절단은 항목 단위로.** 관찰 렌더가 JSON을 바이트로 자르면 마지막 카드가 값 중간에서
    끊겨, 모델이 잘린 recordRef를 그대로 인용하고 게이트가 거부한다(실재하는 출처인데도).
    `fit_result_content`가 항목을 통째로 덜어내고 뺀 개수를 알린다. 도구의 목록 키는 `items`로
    통일했다 — 저장 payload 규약과 같은 어휘여야 하고, 항목 단위 축소도 그 이름을 본다.
  - **모델 행동 제약은 프롬프트가 아니라 도메인이 건다.** 온디맨드 턴에서 저장에 성공한 뒤
    모델이 같은 산출물을 `max_steps`까지 반복 저장했다(전부 게이트 통과 — 결과는 같고 호출만
    늘었다). 프롬프트 문구·시스템 노트로는 막히지 않아, 저장 성공 후 도구 호출 제안이 오면
    도메인이 턴을 끊는다. 답장과 산출물 참조는 그대로 나간다.
  - **설명은 검증 모델에서 갈라지면 안 된다.** 산문 형태 설명은 모델 필드가 바뀌어도 조용히
    남는다 — 필수 필드 이름과 중첩 객체의 내부 필드가 설명에 드러나는지 테스트로 못 박았다
    (실제로 `evidence.abstain_reason`·`coverage` 객체, `experiment_plan`의 비어 있지 않은
    배열 조건이 빠져 있었고 그대로 보내면 거부되는 상태였다).
- Verification: 로컬 실스택 재검증 — 필수 3종 전부 저장·`completed`(수정 전에는 24~60반복
  전부 `partial`), 도구 호출 9회. 온디맨드 턴 2회(생성·갱신) 모두 산출물 참조가 걸린 답장,
  저장 호출 4회 → 1회.
