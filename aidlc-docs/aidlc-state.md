...
- **로컬 실스택 검증(2026-07-27)** — 잡 3건·대화 턴 5회. 확인된 것과 고친 것:
  ...
  - ⚠️ 같은 재측정에서 드러난 **반복 호출**: view_figure 6회 중 서로 다른 대상은 2개뿐이고
    (한 논문의 목록·`figure:0`), `corpus_search`는 거의 같은 질의로 13회 돌아 검색 캡
    12/12를 소진했다. 원인은 관찰 렌더가 결과 줄에 **과거 호출의 인자를 싣지 않는다**는
    것 — 모델은 `[6] corpus_search (ok)` + 결과 JSON만 보고 그게 어떤 질의였는지 모른다.
    ⑤3 이전 잡에서도 나타나던 루프 공통 특성이라 ⑤3 범위 밖의 별건으로 분리한다
    (`ToolResultView`에 `summarize_args` 결과를 실어 렌더).
