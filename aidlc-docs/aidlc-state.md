...
- **로컬 실스택 검증(2026-07-27)** — 잡 2건·대화 턴 5회. 확인된 것과 고친 것:
  ...
  - 🔧 **도구 설명의 "꼭 필요한 것만"이 이미지 열기를 예외로 프레이밍**해 모델이 목록만
    ...
    재측정에서 이미지를 여는 것을 확인.
  - ⚠️ **자율 루프는 view_figure를 스스로 쓰지 않았다(2잡 중 0회).** 필수 산출물 완성이
    강한 목표라 그림 열람이 그것을 진전시키지 않기 때문으로 보인다. 대화 턴 경로는 정상
    동작하므로 기능은 살아 있으나, 루프 프롬프트 튜닝은 **측정 수단을 갖춘 뒤** 할 후속
    과제로 남긴다(튜닝 1회마다 잡 실행 비용).
