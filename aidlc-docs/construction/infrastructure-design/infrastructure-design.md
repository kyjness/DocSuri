...
| 원칙 | 내용 |
|---|---|
| **비용 상한** | NFR-C1 = **$1,600/월 시스템 전역** (팀 AWS 크레딧 전액). 본 문서 §6에서 유닛별 배분. ⚠️ *(2026-08-13 — 이 값은 **실서비스 기준으로 보존**한다. 2~3개월 데모 배포는 개인 계정 프리티어 예산으로 별도 산정하며 **§12** 참조.)* |
...
## 12. 데모 배포 토폴로지 — Lightsail 단일 인스턴스 (2026-08-13 추가)

> **§1~§11은 실서비스 기준 명세로 그대로 유효하다.** 본 절은 그것을 대체하지 않고, **2~3개월짜리 데모
> 배포**를 위한 **별도의 축소 토폴로지**를 정의한다. 근거는
> `inception/requirements/requirement-verification-questions-corpus-and-deployment.md`(Q1~Q9 확정)이며
> `requirements.md`의 「개정 (2026-08-13)」 블록과 정합한다.

### 12.1. 전제 변경

| 항목 | §0 (실서비스) | 데모 배포 |
|---|---|---|
| 비용 근거 | NFR-C1 **$1,600/월** — 팀 AWS 크레딧 전액 | **신규 개인 계정 프리티어 크레딧** (최대 $200, 6개월 또는 소진 시까지) |
| DocSuri 몫 | — | **3개월 $72** |
| 통제 방식 | 월 상한 + 서킷 브레이커 | **총액 + AWS Budgets 알람** (데모 배포에서는 "다 쓰면 종료"가 실제 제약) |
| 리전 | `ap-northeast-2` | 동일 |

**계정 공유**: 같은 계정에서 puppytalk을 함께 배포한다. Lightsail **무료 번들은 계정당 1개**이고
대상이 최대 2 GB($12) 등급까지이므로, **무료 번들은 puppytalk(2 GB)이 사용하고 DocSuri는 크레딧으로
지불**한다. DocSuri는 검색엔진(OpenSearch)이 붙어 2 GB로 동작하지 않는다.

> **⚠ 이 절의 박스 사양은 2026-08-15에 개정됐다 — 아래 표는 그 개정 전 값이다.**
> 정본은 `inception/requirements/requirement-verification-questions-corpus-and-deployment.md`
> Q1이고, 확정값은 **8 GB `large_3_0` · 3개월 $144 · 편수 천장 약 3,500편**이다.
> 4 GB로 시작하려던 근거(편수 천장 약 4,500편)는 **인메모리 색인을 잰 값**이라 폐기됐다 —
> 946편을 실제로 색인해 재니 **편당 1.73 MB**였고, on_disk 모드에서는 k-NN 그래프가 RAM에
> 상주하지 않아(`graph_memory_usage: 0`) "k-NN 예산 → 편수" 산정 자체가 성립하지 않는다.
> 제약은 RAM이 아니라 **디스크·페이지캐시**다. 4 GB의 실제 천장은 약 **1,750편**이라 기반
> 논문 1,500편만으로 차고 최근분 자리가 없다.
>
> 개정이 질문지에만 반영되고 이 절·`requirements.md`·`solo-roadmap.md`에 **전파되지 않아
> 열흘 동안 세 문서가 폐기된 수치를 들고 있었다**(2026-08-25 발견). 아래 표를 지우지 않고
> 남기는 이유는 §12.3의 산정식이 왜 틀렸는지가 그 자체로 기록이기 때문이다.

### 12.2. 리소스

| 리소스 | 사양 |
|---|---|
| 인스턴스 | ~~Lightsail `medium_3_0` — 4 GB RAM, $24/월~~ → **`large_3_0` — 8 GB RAM · 2 vCPU · SSD 160 GB**, $44/월 (2026-08-15 개정) |
| OS | Amazon Linux 2023 |
| 구성 | 로컬 `docker compose` 구성을 그대로 이관 — Caddy(TLS 종단) · 프론트 · 백엔드 · postgres · redis · **opensearch** |
| 고정 IP | Lightsail static IP |
| 오브젝트 스토어 | 실 S3 (로컬 s3proxy 대체) |
| 방화벽 | 80/443 공개, 22는 배포 경로에 한정 |

**§1~§9 대비 제거되는 것**: ECS Fargate(6 서비스) · ALB · RDS · ElastiCache · OpenSearch Domain ·
EventBridge · SQS 14종. 큐가 필요한 경로는 로컬 구성과 동일하게 Redis로 대체한다.

**앱 코드는 변경하지 않는다** — 헥사고날 포트/어댑터 구조이므로 배선(환경변수)만 교체한다.

### 12.3. 메모리 배분 — 편수 천장의 근거

프로덕션 이미지(`backend/Dockerfile` uvicorn · `frontend/Dockerfile` Next standalone)를 그대로 빌드해
기동하고 페이지·API 요청을 태운 뒤 `docker stats`로 측정한 값이다(2026-08-13).

| 컨테이너 | 실측 |
|---|---|
| 백엔드 (uvicorn, 10개 모듈 전부 마운트) | **155 MB** |
| 프론트 (Next standalone) | **56 MB** |
| postgres | 34 MB |
| redis | 7 MB |
| **앱 합계** | **약 250 MB** |

산정식 **`박스 RAM − 앱 몫 = OpenSearch 예산 → k-NN 예산 → 편수`**. 앱 몫은 실측 250 MB가 아니라
**OS·Caddy·에이전트 동시 부하 여유를 포함해 0.9 GB**로 잡는다.

| | 값 |
|---|---|
| OpenSearch 몫 | 약 3.1 GB |
| k-NN 예산 | 약 1.8 GB |
| **편수 천장** | ~~약 4,500편~~ → **약 3,500편** (8 GB · **편당 1.73 MB 실측** · 완화책 미적용). 위 k-NN 예산 산정은 on_disk 색인에서 성립하지 않는다 |

**청크 크기·출력 차원 완화책은 적용하지 않는다**(질문지 Q5=A) — 검색 정밀도 하락이 정량 측정되지
않았기 때문이다. 편수가 부족하면 완화책이 아니라 **인스턴스 등급을 올린다**(Lightsail은 스냅샷으로
상위 등급 이전이 가능하므로 되돌릴 수 있는 결정이다. 참고: `large_3_0` 8 GB = 약 11,000편, $44/월).

### 12.4. 운영

- **파싱 스택(GROBID 1.84 GB · Docling)은 배포 박스에 올리지 않는다.** 파싱·색인 구축은 로컬에서
  수행하고 배포에는 **색인 결과만** 올린다(질문지 Q7=B — 배포 기간 중 증분 수집 없음).
  이 결정이 없으면 4 GB로는 성립하지 않는다.
- 단일 인스턴스이므로 **단일 장애점이며 오토스케일·무중단 배포가 없다.** 데모 배포에서 수용한다.
- 배포 방식은 puppytalk 선례(`puppytalk-infra/demo/` Terraform + SSH 배포)를 참고한다.

### 12.5. 보존

`ops/cdk/` 스택 9종과 본 문서 §1~§11은 **실서비스 토폴로지 설계 기록으로 보존**하며 이번 배포에
사용하지 않는다. 삭제하지 않는다.
