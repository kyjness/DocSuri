...
| 원칙 | 내용 |
|---|---|
| **비용 상한** | NFR-C1 = **$1,600/월 시스템 전역** (팀 AWS 크레딧 전액). 본 문서 §6에서 유닛별 배분. ⚠️ *(2026-08-13 — 이 값은 **실서비스 기준으로 보존**한다. 2~3개월 한시 배포는 개인 계정 프리티어 예산으로 별도 산정하며 **§12** 참조.)* |
...
## 12. 한시 배포 토폴로지 — Lightsail 단일 인스턴스 (2026-08-13 추가)

> **§1~§11은 실서비스 기준 명세로 그대로 유효하다.** 본 절은 그것을 대체하지 않고, **2~3개월 한시
...
| DocSuri 몫 | — | **3개월 $72** |
| 통제 방식 | 월 상한 + 서킷 브레이커 | **총액 + AWS Budgets 알람** (한시 배포에서는 "다 쓰면 종료"가 실제 제약) |
...
- 단일 인스턴스이므로 **단일 장애점이며 오토스케일·무중단 배포가 없다.** 한시 배포에서 수용한다.
