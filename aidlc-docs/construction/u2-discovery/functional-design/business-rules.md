...
| **BR-5b (Cross-Encoder 재랭킹)** *(2026-07-06 페이즈 7)* | retrieve→융합 상위 **M** 후보를 cross-encoder 재랭킹 모델로 재정렬해 `ranking_score`를 덮어씀(head만; tail 불변). retrieve 폭(150)은 불변. **M=100(lite)/150(full)** *(2026-08-15 상향, 종전 30/50)* — head만 남기고 tail을 버리므로 M은 재랭킹 폭이자 **ranker가 보는 후보 풀의 크기**이고, 융합이 공급하는 ~150~300편을 30/50으로 자르면 재랭킹을 켜는 것 자체가 후보를 좁히며 표시 깊이도 M에서 끊긴다. 상향 근거는 이 손실 복구이지 정량 평가가 아니며(라벨셋 부재), **최적 M 탐색은 여전히 정량 신호 확보 후**다. 실측(2026-08-15 도쿄 Cohere Rerank v3.5): 지연이 M에 거의 무관(30~150건 1.1~1.9s)이라 P50<3s lite 경로에서 상향 비용이 사실상 없음. 제약은 M이 아니라 **계정 요청속도 쿼터**(스로틀→baseline fail-soft). 오케스트레이터에서 게이트(I/O 결정): 어댑터 미배선(feature off)·예산 저하(`rerank_enabled`=False, RERANK_OFF/LEXICAL_ONLY)면 skip, **어댑터 실패는 fail-soft로 베이스라인(RRF) 유지 — 재랭킹은 응답을 절대 막거나 저하 모드로 만들지 않음**(순위 품질 향상일 뿐). 적용=lite·full 둘 다. | FR-3, NFR-C1, NFR-R2 |
...
| **BR-13 (인증 전제)** | `POST /api/search`는 **인증 필수**(deny-by-default, SEC-8). authn 강제는 U6 게이트웨이; U2는 `RequestContext.authSession` 신뢰·userId 사용(SearchExecuted). 미인증은 게이트웨이 401. *(2026-07 개정 — 제품 결정: 익명 검색 허용. 게이트웨이가 `/api/search`를 auth-optional로 운용하며, 미인증 요청은 서버 통제 고정 anonymous id로 해석(클라이언트 `X-User-Id` 헤더는 prod에서 불신 — 스푸핑으로 타 계정 이력 주입 방지, SEC-8). authn 강제 주체가 U6이라는 단일권위는 불변. 근거 커밋: 568677c·09a496a.)* | SEC-8, FR-10, Q5=A→개정 |
