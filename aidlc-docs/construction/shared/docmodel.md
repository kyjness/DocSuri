...
- **PDF/GROBID 경로**: rich HTML이 없는 논문(비arXiv 포함)은 `sourceTier=pdf`로 GROBID **TEI를 구조화 파싱**한다(`parse_tei_to_docmodel`): `<div>/<head>`→섹션, `<p>`→문단(인라인 ref/수식 텍스트 보존), `<table>`→**구조화 데이터(rows/cells)**(좌표 보유 시 page-crop `assetRef` 폴백 가산·아래), `<figure>`→이미지 `assetRef`, block `<formula>`→이미지 `assetRef`(type=`formula`). 신뢰 가능한 LaTeX가 없으므로 수식은 LaTeX 대신 page-crop 이미지로 degrade(검색 비대상). figure/table은 GROBID이 묶어 내보내므로 `coords`(page,y)로 정렬해 배치한다. TEI 결손·파싱 실패 시에만 **단일 paragraph block의 평탄 DocModel**(`parse_text_to_docmodel`)로 last-rung 폴백한다(메트릭 노출). 그림·수식·표 이미지는 TEI `coords`를 같은 walk에서 크롭 spec(블록과 동일 assetId)으로 수집해, 멀티모달 활성 시 PDF를 page-crop해 채운다(ordinal 정합 보장·best-effort). **표는 구조화 데이터가 주 표현으로 남고**(검색·근거화·에이전트 대상), 좌표가 있을 때만 보조 폴백 이미지를 `assetRef`로 가산한다(D8): GROBID 셀 복원이 깨진 표를 이후 비전 리더가 이미지로 재판독할 수 있도록 — 데이터를 대체하지 않는다.
