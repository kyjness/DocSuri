...
- **Boost Magnitude Contract**: U9가 산출하는 개별 boost 값은 `[-0.1, +0.1]` 구간 내에 존재해야 하며, 전체 부스트의 총합은 최대 `0.2`를 넘지 않아야 한다(상대적 가중치 기준).
- 이 계약은 U2가 제공하는 검색 기본 적합성(relevance score)의 순서를 극단적으로 뒤집지 않고, 상위 30% 이내의 결과들에 한정하여 미세 재정렬(rerank)을 유도하는 기능적 제약(Functional Constraint)이다.
