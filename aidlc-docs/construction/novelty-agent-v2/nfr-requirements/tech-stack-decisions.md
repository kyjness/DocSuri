...
| TD-NV2-3 | LLM tool-calling 포트 | **Bedrock Anthropic tool-calling 어댑터**(2026-08-16 정정; 당시 OpenAI function-calling 신규) | 어댑터 교체로 흡수됨 — 예고대로 루프 코어 무변경. 프로바이더 스위치는 제거(단일 어댑터) |
...
- **OpenAI 1차(TD-NV2-3, 당시)**: 로컬 이관에서 이미 임베딩·요약 프로바이더로 검증됨. tool-calling 어댑터만 신규. 프로바이더 비교 실험은 어댑터 추가만으로 가능함을 계약 수준에서 보장. — **결과(2026-08-16)**: 배포 재개로 Bedrock 어댑터로 교체됐고, 그 교체가 루프 코어·프롬프트 무변경으로 끝나 이 보장이 실증됐다.
