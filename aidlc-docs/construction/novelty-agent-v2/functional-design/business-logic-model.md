...
**관찰의 결과 줄은 인자를 함께 싣는다** — `[seq] 도구명(인자) (상태)`. 결과만 보여주면
모델은 그것이 **어떤 호출에서 나왔는지** 알 수 없어 같은 질의·같은 자산을 반복하고,
그 반복이 캡을 태워 필수 산출물을 완성하지 못한 채 종료한다(실측: 검색 캡 소진 →
partial). 값은 트레이스가 쓰는 인자 요약을 재사용하고, 모델이 쓴 값이므로 오류 문구와
같은 무해화 파이프라인을 거친다.
