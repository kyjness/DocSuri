...
**타입 분기를 두지 않는다**: `paper_asset`에 행이 존재한다는 사실 자체가 판정이다.
구조화에 성공한 표는 DocModel에 rows/cells로 실려 crop이 없고(표 crop 행은 비-HTML
폴백 티어에서만 생긴다), 수식은 `latex`와 `assetRef` 중 **하나만** 렌더 소스이므로
(`docmodel.schema.json`) 수식 crop 행의 존재가 곧 LaTeX 복원 실패를 뜻한다. 따라서
§3 표의 "표는 텍스트로, 수식은 LaTeX 1차·crop 폴백"은 DocModel을 따로 읽지 않아도
매니페스트만으로 성립한다.
