...
| **BR-29 (전문 추출 소스·형식)** | **결정 D 활성**(FD plan `사후 결정` Q18=D; #139 근원=추출 소스·형식 미설계). **[개정 2026-08-10 — 소스 사다리 축소]** 전문은 **ar5iv HTML → PDF 폴백**으로 취득한다. native arXiv HTML(`arxiv.org/html`)은 사다리에서 **완전 제거** — 운영 검수에서 native 산출물의 화면 품질 불량이 반복 확인됐고, LaTeXML 변환 실패 시 원시 TeX가 평문에 그대로 샌다(2502.10208: `ltx_ERROR` 167개·본문 90% 소실이 종전 가드를 통과). HTML은 *가장 깨끗한 평문을 뽑는 소스*, PDF는 폴백(개정 전 실측: native 단독 34%·ar5iv 포함 90% — native 제거로 잃는 커버리지는 PDF 폴백이 흡수). **보관=정규화 평문(`.txt`)** — **인덱스 청킹·검색 투영용으로 유지**(인덱싱 권위; Q4: doc-model=진실원천, `.txt`=파생 평문). **e-print(LaTeX) 미해제 디코딩 금지** — gzip/tar 페이로드를 텍스트로 디코딩하지 않음(`arxiv.py:73` 결함 제거); 단 doc-model 폴백 단계에서 *해제 후* LaTeX 파싱은 허용(BR-30, Q6 폴백 사다리). HTML 부재는 폴백(거부 아님); 추출·평문화 실패는 BR-15 분류(PARSE/FETCH). **피벗(doc-model)**: 수식·표·그림의 리치 렌더/보관은 더 이상 범위 밖이 아님 — **doc-model로 구조화**(BR-30)하고 자체 리치뷰가 렌더(D4). 앵커 하이라이트 계약 유지(BR-SF). on-demand 비전은 에이전트 단계(D5). | **D**, Q4, Q6, SEC-5, BR-15/19/20/30 |
| **BR-30 (doc-model 구조·생성, 피벗 2026-06-23; Corpus 개정 2026-06-26)** | **결정 D1·D2·D6 활성**(SSOT=`construction/plans/docmodel-foundation-pivot-plan.md`). doc-model = HTML/GROBID 결과의 **결정적 파싱 → JSON**(`doc-model/{paperId}/v{ver}.json`): **`fullText` 전문 텍스트 투영본** · 섹션/블록·앵커 · **표=구조화 데이터(rows/cols)** · **수식=LaTeX(MathML 변환)** · **그림/표이미지=webp `assetId` 참조**(픽셀·메타는 §6/FR-17 산출물 재사용 — **재추출 0**). **LLM 추출 금지**(표 숫자 환각 방지) — 결정적 파서만, 동일 소스→동일 doc-model(P7). **Corpus phase-1 생성=eager + `(paperId, version)` 캐시**(BR-C6): legacy lazy on-demand는 누락분·재빌드·백필 호환 경로다. **[개정 2026-08-10 — Q6 사다리 재확정]** 생성 사다리=`ar5iv HTML(품질검사) → arXiv PDF → GROBID(품질검사) → 코퍼스 제외`. (1) **native HTML 제거** — BR-29 개정과 동일 근거. (2) **PDF 평문 doc-model 제거** — 섹션·블록 없는 글자 뭉치(`parse_text_to_docmodel` 산출, 섹션 2·문단 2)는 doc-model로 서빙하지 않는다; 그 자리는 GROBID 구조화 빌드가 대신한다. (3) **품질검사를 ar5iv에도 건다** — 깨짐은 native 특성이 아니라 LaTeXML 특성(실측: 동일 10편에서 ar5iv 3편·native 4편에 `ltx_ERROR`). 검사는 **상대 커버리지**(파서가 만든 `fullText` ÷ 소스 페이지 평문 길이)로 하고, 기존 절대 바닥(본문 500자)과 함께 건다 — 절대 바닥은 "초록만 남은" 절단은 잡지만 "본문은 길고 대부분 소실"은 못 잡는다. **분모는 구조적 편향 제거 후** 계산한다(2026-08-10 리뷰 반영): 참고문헌(`ltx_bibliography`, 블록 트리에 설계상 미투영, 비중 무한)과 MathML `<annotation>`(수식 TeX 원문의 이중 계상, 최대 22% 실측)을 빼지 않으면 참고문헌 많은 수학 논문이 변환 건강과 무관하게 문턱으로 흘러내린다 — 벌칙이 '제외'이므로 허용 불가. 문턱은 코퍼스 실측으로 확정하고 상수 주석에 근거를 남긴다. **`ltx_ERROR` 개수 세기는 기각**(2026-08-10 실측): 미해결 매크로 하나가 **사용 횟수만큼** 오류를 찍으므로 분자만 선형으로 커지고, 멀쩡한 논문(2310.04047·2501.08626)이 표본의 3.3%나 오탈락했다. 그 게이트가 잡아낸 진짜 파손은 전부 절대 바닥이 이미 잡고 있었다. (4) **끝내 실패한 논문은 코퍼스에 넣지 않는다** — 색인하지 않고, 전문 객체도 **애초에 저장하지 않는다**(빈 본문 검색 노출 금지). 그래서 doc-model 빌드는 `put_full_text` **앞**에 온다(dedup 단락·`begin_upsert` 뒤는 그대로) — 쓰고 지우면 제외 논문마다 S3 왕복이 두 번이고, 일시 장애 재배달마다 반복되며, 무엇보다 **이미 색인된 논문의 재파싱**에서 기존 청크는 남은 채 전문만 지워져 전문 보기가 깨진다. (5) **S2/OpenAlex는 수집 전용** — arXiv 논문의 파싱 폴백으로는 무용(OpenAlex `locations`가 arXiv 프리프린트에 arXiv PDF만 반환, 실측 2512.08879·2504.11054); 비-arXiv 경로(`PDF → GROBID`)는 기존대로. e-print LaTeX 단은 미구현 상태 그대로 사다리 표기에서 제외. 알고리즘은 BLM §7. | **D1/D2/D6**, Q4/Q5/Q6, BR-29, FR-17 |
...
| **BR-23a (크롭 프레이밍 — 2026-08-10 신설)** | `page-crop`이 담는 것은 **캡션을 뺀 float의 내용**이다. GROBID의 float `coords`는 캡션의 텍스트 줄 나열 + 내용 영역이고, 그걸 통째로 union하면 이미지의 10~69%가 캡션이 된다(뷰어는 `<figcaption>`으로 캡션을 따로 렌더한다). 규칙: **내용 영역은 `<graphic>`(그림)·`<table>`(표)이 정한다**(페이지도 이쪽이 정본 — 캡션 띠가 앞 페이지에 있는 표가 실재한다). y는 내용과 겹치는 영역만, x는 float 자신의 폭(서브그림의 `<graphic>`은 단보다 좁을 수 있다). 표의 `<table>` 상자는 자주 붕괴하므로(50편 스윕에서 표 크롭 211건 중 80건이 열 구조 행을 잃었다) **렌더 시 float 상자를 상한으로 행을 따라 되키운다**(단 사이 홈 72pt에서 끊는다). 알고리즘은 GROBID가 여러 `<formula>`로 쪼개므로 **원소 전체가 한 리스팅으로 들어갈 때에 한해 조각의 좌표를 합쳐 통째로** 크롭한다. `<graphic>`·`<table>`도 없고 pdfium 회수도 실패해 **영역이 캡션 말고 아무것도 말하지 않으면 자산을 저장하지 않는다**(뷰어가 플레이스홀더+캡션으로 렌더 — 캡션 사진을 그림이라고 보여주지 않는다). | 운영 검수, ⑧-1.5 후속 |
| **BR-23b (외부 소스 fetch 규약 — 2026-08-10 신설)** | **모든** 소스로 나가는 요청은 arXiv·ar5iv를 포함해 **우리를 식별한다**(`User-Agent` + 연락처, `DOCSURI_CONTACT_EMAIL`). 익명 요청에 Springer·Nature는 200 + 3KB 랜딩 HTML을, Cureus는 403을 돌려준다(실측). **본문이 PDF가 아니면(`%PDF-` 아님) 영구 실패**다 — 랜딩 페이지는 다시 받아도 랜딩 페이지이므로 retriable로 두면 DLQ에 무한 적재되고, 그 바이트가 **원본 캐시에 PDF로 저장되면 이후 모든 재파싱에 되살아난다**(따라서 검사 실패분은 캐시하지 않고, **캐시에서 읽은 본문도 같은 검사를 통과해야 한다** — 검사 이전에 오염된 항목은 미스로 취급해 재수확이 덮어쓴다; 검사는 `http_limits.is_pdf_payload` 한 곳). 대체 사본은 **primary와 라이선스가 같은 것만** 후보다 — 레코드는 라이선스를 하나만 기록하고 하류 OA 게이트도 그것만 보므로, 다른 라이선스의 바이트를 받으면 제공된 적 없는 조건으로 기록하게 된다. 검색 어구는 **따옴표로 감싼 선언**이다(`"computer vision" | "machine learning"`) — 따옴표 없는 `|`/`OR`는 인접 단어에 묶여 구가 사라진다(S2 문법 실측). 한 논문의 **다른 사본**이 있으면 순회하되(상한 3, 레코드에 싣는 대체 URL도 같은 상한) 429/5xx는 순회하지 않고, **후보는 위치마다 라이선스를 따로 검사**한다(리포지터리 사본은 출판사 조건을 물려받지 않는다 — BR-1). 수확 슬라이스는 두 소스가 **같은 분야**를 보게 한다(S2 `fieldsOfStudy`, OpenAlex `primary_topic.field.id`)와 카테고리는 **선언(OR)** 이다. | 기동 검증, BR-1/BR-18 |
