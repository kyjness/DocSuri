...
| **BR-23b (외부 소스 fetch 규약 — 2026-08-10 신설)** | 제3자 출판사로 나가는 요청은 **우리를 식별한다**(`User-Agent` + 연락처, `DOCSURI_CONTACT_EMAIL`). 익명 요청에 Springer·Nature는 200 + 3KB 랜딩 HTML을, Cureus는 403을 돌려준다(실측). **본문이 PDF가 아니면 영구 실패**다 — 랜딩 페이지는 다시 받아도 랜딩 페이지이므로 retriable로 두면 DLQ에 무한 적재된다. 한 논문의 **다른 사본**이 있으면 순회하되(상한 3) 429/5xx는 순회하지 않고, **후보는 위치마다 라이선스를 따로 검사**한다(리포지터리 사본은 출판사 조건을 물려받지 않는다 — BR-1). 수확 슬라이스는 두 소스가 **같은 분야**를 보게 한다(S2 `fieldsOfStudy`, OpenAlex `primary_topic.field.id`)와 카테고리는 **선언(OR)** 이다. | 기동 검증, BR-1/BR-18 |
