...
> **우선순위**: 본 섹션은 U1 Corpus 최신 NFR Design이다. 아래 §1~§7의 arXiv-only, Cohere v4(현 임베딩 모델), 단일 watermark, legacy chunk/index 설명과 충돌하면 **본 섹션을 우선한다**.
...
* **Bedrock**: 특정 Cohere Embed Multilingual v4 모델 ARN에 대한 `InvokeModel`만 허용.
