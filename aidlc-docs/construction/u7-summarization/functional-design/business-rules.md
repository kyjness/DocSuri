...
- **초록(s0)은 앵커 대상 제외(개정)**: 리더(전문 뷰)는 초록을 본문·TOC에서 숨긴다(중복 방지 — 별도 `초록`/`초록 번역` 면). 따라서 refiner가 doc-model을 투영할 때 **초록 섹션(doc-model id `s0`)은 프롬프트 본문엔 그대로 포함하되(요약 근거로 계속 사용) 앵커 색인(`RefinedSource.sections`)에서만 제외** → 게이트가 어떤 필드의 앵커든 초록으로 해석하지 않는다(리더의 s0 필터와 대칭 — 사용자가 스크롤로 갈 수 없는 위치는 인용처로 제시하지 않음). 한 줄 요약은 성격상 초록을 근거로 하므로 표시할 앵커가 자연히 없어지고, U5도 한 줄 요약 필드에는 출처 칩을 렌더하지 않는다(BR-SF 표시 규칙과 정합).
