...
def discover_source_code(project_root: Path) -> list[Path]:
    ...
    # DocSuri fork: add DocSuri monorepo roots so source code is discovered.
    # backend/ (modules/*, middleware/, db.py), ingestion/ (src/...), ops/ (src/...),
    # shared/ (python/src/...), frontend/ (app/, lib/, *.ts). All scanned recursively below.
    source_dirs = [
        "src", "lib", "app", "packages",
        "backend", "ingestion", "ops", "shared", "frontend",
    ]
    exclude_patterns = {
